...
| 步骤 | 真名 | 代码片段 |
|---|---|---|
| ① 入站检查 | `createDaemon (Lyt)`；`assertWsChannelIdentityParams (k0e)`；`isSessionArchiving (or)`；`resolveIngressWorkspace (x0e)` | `over WebSocket requires params.channel_id`（`assertWsChannelIdentityParams`）；`code: -32011`（`createDaemon`）；`code: -32010`（`createDaemon`） |
...
| ⑨ 执行记录与收尾 | `drainSessionMailbox (KSe)`；`createDrainExecutionEventRecorder (ake)`；`emitDrainOutputRecords ($c)`；`patchSessionRuntimeState (et)` | `type: "agent.tool_use"`（`createDrainExecutionEventRecorder`）；`type: "agent.result"`（`emitDrainOutputRecords`）；`last_event_id: Y.event.id`（`drainSessionMailbox`） |
...
三点需要单独说明。第一，唤醒会话的是入站处理函数在指针写入之后发出的 `session.wake`，`appendBeforeExecuteGateway (Kle)` 发出的 `spine.event` 总线事件在 bundle 中没有订阅者（confirmed，第 6.2 节）。第二，第 ⑨ 步的三类执行记录由 drain 路径上的两个辅助函数写入：`createDrainExecutionEventRecorder (ake)` 在引擎每报告一次工具调用或工具结果时追加一条 `agent.tool_use` 或 `agent.tool_result`，并跳过标为 ephemeral 的工具调用，判断条件是 `o.type === "tool_use" && !o.ephemeral`（`createDrainExecutionEventRecorder`）；`emitDrainOutputRecords ($c)` 对这一轮的每个投递目标会话先写一条出站记录，再追加一条 `agent.result`（confirmed；各事件类型的写入方见第 5.2 节）。第三，后台分区改写记忆板由分区提示词规定，代码只负责按心跳调度分区会话和在前台装配时读入记忆板：`subconscious/intuition-weaver/CLAUDE.md` 写明它是 `memory/CLAUDE.md` 的唯一写入者（confirmed，提示词原文；分区调度见第 11 节，记忆板的分工见第 12.4 节）。
...
第 6 层由 `renderJobMissionBlock (Ahe)` 生成，分两种写法（confirmed）。无状态 job（`stateless`）的文本告诉模型它运行在一个全新上下文里，上文没有历史，跨次运行需要的状态必须写进文件；有状态 job 的文本告诉模型上文每一轮都是同一任务书的一次过去执行，应当当作参考而不是对话。job frontmatter 带 `acceptance` 时，`renderJobMissionBlock (Ahe)` 调用 `renderJobAcceptanceBlock (xrt)` 生成一段文本追加在任务书之后，`acceptance` 去掉首尾空白后为空则不追加（confirmed）。这段文本是一段说明加 `<acceptance>` 块：说明写明下列条件全部成立时 job 才算完成，要求模型结束前核对这些条件、写明哪些没能满足而不是不加说明地结束，并把条件没有要求的工作视为范围之外（confirmed）。job 的定义与调度见第 10 节。
...
- 文件的 frontmatter 不进入展开后的文本。HTML 注释只有在文件顶层单独成块时才被删去：`stripBoardHtmlComments (vgt)` 只处理 Markdown 词法分析得到的顶层记号，写在段落里或列表、引用块内部的注释保留在展开后的文本中（confirmed）。收集引用的 `collectBoardIncludePaths (wgt)` 递归遍历全部记号，跳过代码块、行内代码和任何位置的 HTML 注释，所以这些地方的 `@` 都不算引用（confirmed）。路径中 `#` 之后的部分被去掉，`\ ` 表示空格；相对路径以引用它的文件的真实路径所在目录为基准，`~/` 展开为用户主目录（confirmed）。
...
表中各块的顺序、tag 与注入条件都由 `buildTransientUserBlocks (eke)` 和 drain 代码确认（confirmed）。其中 `smart-compact-notice`、`time-context`、`job-tick`、`board-updated` 四类块的正文分别由 `renderSmartCompactNoticeBlock (Qdt)`、`renderTimeGapContextBlock (eft)`、`renderJobTickBlock (nft)`、`renderBoardUpdatedHint (PSe)` 生成，表中"内容"一列与这四个函数输出的字段一致（confirmed）。`smart-compact-notice` 注入条件的后两项由 `resolvePendingCompactNotice (Xdt)` 判断：会话状态中的 `last_event_at` 晚于 `last_compact_at`，或压缩统计的测量时间不等于 `last_compact_at` 时，它不返回提示（confirmed）。

时间块的阈值来自 effective config 的 `time_gap_minutes`，缺省 60 分钟，设为 0 或负数时关闭；`computeTimeGapContext (QSe)` 只在渠道会话、这一批里有用户消息、会话有上次事件时间、且这次 drain 中还没有注入过时间块时构造时间差对象，并把阈值一并交给 `renderTimeGapContextBlock (eft)`；后者在时间差小于阈值时不输出文本（confirmed）。重启提示的判定由 `decideRestartHintInjection (Bbe)` 完成，非渠道会话直接判为不适用，所以 job、分区、system 会话收不到这个块（confirmed）。`job-tick` 只由 `drainSessionMailbox (KSe)` 的逐条处理路径传入，合并窗口路径 `prepareDrainTurnContext (SH)` 不传这一项（confirmed）；同一条路径在构造了 `job-tick` 时把用户原文换成上表中的固定句（confirmed）。
...
四个引擎的适配器接收的都是 2.1 那段六层文本，它们只做格式转换；文本是否在每轮都送达引擎，取决于该引擎的会话或进程何时建立。下表列出四者的差别（confirmed，逐项证据见下文与证据表）。
...
**pi。** 系统提示只在 pi worker 进程启动时生效一次（confirmed，daemon 与 pi-worker 两侧都如此）。pi-worker 只处理第一个 "init" 帧，之后的 "init" 帧记一条 "duplicate init ignored" 后忽略，而它读取 "system_prompt" 的地方只在初始化处理里；`override` 模式时用这段文本替换 pi 的默认系统提示，并从 pi 默认系统提示里截取 "Available tools:" 到 "Pi documentation" 之间的工具说明段追加在后面；`append` 模式时经 "appendSystemPromptOverride" 把文本追加在 pi 默认系统提示之后（以上均为 pi-worker.js 字面量，confirmed）。daemon 侧，会话管理器默认用 `createPiWorkerAdapter (TO)` 构造 pi 适配器；它只在 worker 进程尚未启动时读取传入的系统提示，经 `buildPiSystemPromptSpec (qft)` 把字符串记为 `override`、预设对象记为 `append`（去掉首尾空白后为空则不记），写进初始化帧的 `system_prompt` 字段，worker 启动之后传入的系统提示不再读取（confirmed）。因此 `prompt_mode` 的两个取值在 pi 上都起作用。pi-worker 创建会话时传入 "noContextFiles: !0"，按字段名理解是不加载工作目录里的上下文文件，其确切含义取决于 pi 框架，未证实推测。
...
| 任务书分无状态与有状态两种写法，由 `acceptance` 生成的文本可选追加 | `c = n ? Ahe(n, n.stateless === !0) : void 0`（`renderPromptLayers`）；`"FRESH context"`（`renderJobMissionBlock`）；`let n = xrt(e.acceptance)`（`renderJobMissionBlock`）；`"</mission>", ...n ? ["", n] : []`（`renderJobMissionBlock`） | confirmed |
| 验收文本说明条件全部成立才算完成、要求核对并写明未满足项、条件之外的工作不在范围内，条件放在 `<acceptance>` 块里 | `"This job is done when all of the following hold. Verify them before you"`（`renderJobAcceptanceBlock`）；`"silently. Work these do not ask for is out of scope."`（`renderJobAcceptanceBlock`）；`"<acceptance>"`（`renderJobAcceptanceBlock`） | confirmed |
...
| 解析函数拿到的是被引用文件的真实路径，相对路径以它所在目录为基准，`~/` 展开为主目录，`#` 之后去掉，`\ ` 表示空格 | `let a = await ygt(i)`（`resolveBoardIncludes`）；`} = bgt(u, a)`（`resolveBoardIncludes`）；`includePaths: wgt(i, t)`（`parseBoardFileContent`）；`n.add(xgt(a, ha.dirname(t)))`（`collectBoardIncludePaths`）；`ha.join(dgt.homedir(), e.slice(2))`（`resolveBoardIncludePath`）；`let [t] = e.split("#")`（`normalizeBoardIncludeToken`）；`t.replace(/\\ /g, " ")`（`normalizeBoardIncludeToken`） | confirmed |
| frontmatter 不进入文本；只有顶层单独成块的 HTML 注释被删去；代码块、行内代码与 HTML 注释里的 `@` 不算引用 | `(0, VEe.default)(e, _r).content`（`stripBoardIncludeFrontmatter`）；`content: n.includes("<!--") ? vgt(i) : n`（`parseBoardFileContent`）；`i.startsWith("<!--") && i.includes("-->")`（`stripBoardHtmlComments`）；`t += n.raw`（`stripBoardHtmlComments`）；`if (!(s.type === "code" \|\| s.type === "codespan"))`（`collectBoardIncludePaths`）；`let l = a.replace(HEe, "")`（`collectBoardIncludePaths`）；`s.tokens && i(s.tokens), s.items && i(s.items)`（`collectBoardIncludePaths`） | confirmed（否定性证据：`stripBoardHtmlComments` 只遍历传入的顶层记号，不进入 `tokens` 或 `items`，非注释记号按 `raw` 原样拼回） |
...
| pi：系统提示只随第一个初始化帧生效，按两种模式使用 | `g = e.piAdapterFactory ?? TO`（`createSessionManager`）；`!r && !i ? (a = k, l = qft($.systemPrompt))`（`createPiWorkerAdapter`）；`system_prompt: l`（`createPiWorkerAdapter`）；`mode: "override"`（`buildPiSystemPromptSpec`）；`mode: "append"`（`buildPiSystemPromptSpec`） | confirmed（pi-worker 侧的依据是 pi-worker.js 字面量 "init"、"duplicate init ignored"、"system_prompt"、"appendSystemPromptOverride"、"Available tools:"、"Pi documentation"） |
...
改引擎的配置入口也检查历史归属。`channel.spawn` 更新渠道实例描述符时若 `runtime` 变了，`upsertChannelSpawnDescriptor (Ayt)` 先调用 `checkChannelRuntimeRebindConflict (N0e)`，检查这个渠道下是否有会话持有另一个引擎的历史，有就拒绝修改并列出这些会话，要求逐个 `/clear` 之后再改；`session.config` 的处理函数 `applySessionConfigVerb (Tyt)` 设置 `runtime` 时调用同一个检查（confirmed）。
...
| 叠加以锚点事件的有效配置为底 | `h = KV(await Eo(s, "effective_config_ms", async () => YV(e, u.event)), n.jobContext?.sdkConfig)`（`prepareDrainTurnContext`）；`resolveEffectiveChannelConfigForEvent (YV)` | confirmed |
...
Notify 把 `notify_content` 作为一条 `route.deliver` 事件追加进事件日志、在目标会话邮箱写入指针，再以抢占级别 `preempt: never` 唤醒目标，所以不会打断目标正在执行的 turn（confirmed）。Notify 工具体 `runNotifyTool (gg)` 与投递函数里的拒绝条件分三类：调用本身不合格（内容为空、通知链深度达到 5、目标是调用方自己、目标无法解析，或 job 会话省略目标时找不到它的 job 定义、job 没有记录 `owner_session`、记录的 `owner_session` 不是合法的投递目标）；目标会话已归档或正在归档；目标是没有读者的渠道会话（confirmed）。第一类是一般的输入校验，第三类是按"有没有人会读到"做的判断，只作用于渠道会话。此外工具体在调用上下文缺失（没有运行时总线、没有当前会话键）时也报错，这两种情况不由模型的参数决定。

目标由 `target_session_key` 指定。job 会话可以省略它，此时发给 job 文件里的 `owner_session`；其他会话必须指定。显式目标的解析顺序是：已持久化的会话键原样匹配（`if (n in r) return n; let o = await j_e(e, i)`（`resolveNotifyTargetSessionKey`））；否则按显示名别名匹配，唯一命中即用；多个会话同名时报错并按最近活动时间列出这些会话，要求用完整会话键重试；都不命中时报错并附上候选会话（confirmed）。显式目标在解析前、解析出的目标（包括默认的 owner）在解析后，只要等于调用方自己的会话键，调用就直接报错（`Cannot notify self (session_key=${i})`（`runNotifyTool`））（confirmed）。

未命中时的候选会话分三组：与目标相近的会话键、至多三个其他前台会话、至多两个后台会话（`groupNotifyTargetCandidates (q_e)`，confirmed）。"相近"由 `matchNearMissSessionKey (eut)` 判断，比较的不是整个会话键：两个键按 `:` 与 `.` 切成的段数相同、只有最后一段不同且这一段的编辑距离不超过 3；或者两者是可读名相同、哈希不同的 job 会话键（job 可能已重建）；或者最后一段相同（至少 6 个字符）而第一段或段数不同。与目标分属前台、后台不同类别的相近会话单独列出，标题注明投递给它们不会到达用户（confirmed）。
...
通知链深度用来阻止会话之间互相通知形成循环（confirmed）。drain 取本批事件里最大的 `notify_depth`，经批次回调交给会话管理器；Notify 写出的事件深度等于构造工具集时拿到的深度加一，拿到的深度达到上限 5 就报错（`if (a >= M_e) throw new Error(`（`runNotifyTool`））。深度在构造工具集时按值复制，而多数路径并不为每一批重建工具集，所以深度只在部分路径上逐跳累加：
...
- pi 的 `notify.send` 回调不传深度，工具体按 0 计算（`let a = t.notifyDepth ?? 0;`（`runNotifyTool`））。

因此这条循环保护只在 Claude 一次性调用上逐跳生效，在其余路径上只在新建的流式查询、Grok 会话或 Codex 适配器的第一个 turn 上反映当时的深度；对真实循环的实际效果未实测（未证实推测）。

投递由 `deliverRouteEventToSession (Ps)` 完成（confirmed）。目标正在归档或已归档时返回 `session_archiving` 或 `session_archived`，不写事件；否则追加 `route.deliver` 事件、写邮箱指针并发出唤醒，事件类型为 notify 时抢占级别固定为 `never`。返回文字由 `renderNotifyDeliveryReport (V_e)` 生成，逐个目标写明成功、拒绝或出错；只要有一个目标没投成，工具体就把这份报告作为错误抛出，调用方收到以 `Error:` 开头的文字。
...
在 job 会话里，工具只接受 `when`，把本 job 状态文件的 `run_at` 改成这个时间，调度规则不变：周期 job 在原有节奏之外多运行一次，一次性 job 因此不会在本次运行结束时自动归档；第二次调用覆盖前一次的时间（confirmed）。两种用法都由 `parseJobRearmTime (fw)` 解析 `when`，要求相同：接受 `@in <时长>` 或带显式时区的 ISO 时间；不带时区的时间被拒绝，理由是调度按 UTC 计算；不晚于当前时刻的时间也被拒绝，因为一个已经过去的 `run_at` 会在 job 收尾时被当作已消费，一次性 job 的循环会就此结束而没有任何提示（`A re-arm at or before now would be consumed as already spent at finalize, silently ending the loop.`（`parseJobRearmTime`））（confirmed）。
...
Skip 与 QueueOutboundAttachment 只注册给渠道会话，两者处理的都是送给用户的输出（confirmed）。Skip 让这一轮不向用户输出：四个引擎结束这一轮的方式不同，drain 得知"这一轮已被跳过"的途径有三种（Claude 靠 hook 设的标记，pi 靠适配器自己报告，Codex 与 Grok 靠会话状态里的 Skip 记录），但判定之后的处理相同，都是丢弃这一轮的输出，并在下一个用户消息 turn 里告诉模型上一轮被跳过（confirmed）。
...
| system 来源对应 system、meta、cadence 前缀 | `inferActorOriginFromSessionKey (GW)`；`e.startsWith("cadence:")`（`inferActorOriginFromSessionKey`）；`if (e.startsWith("system:")) return {`（`inferActorOriginFromSessionKey`）；`"[session-manager] wake starting actor with inferred origin"`（`createSessionManager`） | confirmed |
...
| Codex 的 schema 由一个转换函数生成，参数一律声明为字符串，只保留描述与必填列表 | `buildCodexStringInputSchema (Xg)`；`inputSchema: Xg(o),`（`buildCodexDynamicTools`）；`type: "string",`（`buildCodexStringInputSchema`）；`o.safeParse?.(void 0)?.success === !0 \|\| n.push(r)`（`buildCodexStringInputSchema`） | confirmed |
...
| 上下文缺失、内容为空、目标是自己、job 找不到定义或 owner 无效时报错 | `"Notify is unavailable: runtime bus is not available in this context."`（`runNotifyTool`）；`"notify_content is required and must not be empty."`（`runNotifyTool`）；`Cannot notify self (session_key=${i})`（`runNotifyTool`）；`no active job definition matched this session_key`（`resolveJobOwnerNotifyTarget`）；`has no owner_session recorded, so there is no default Notify target`（`resolveJobOwnerNotifyTarget`）；`is not a valid route target`（`resolveJobOwnerNotifyTarget`） | confirmed |
...
| 未命中时的候选：三种"相近"规则（末段编辑距离不超过 3、job 可读名相同、末段标识符相同），另列前台与后台会话 | `groupNotifyTargetCandidates (q_e)`；`l = Qat(a, u, z_e);`（`matchNearMissSessionKey`）；`z_e = 3`（`initNotifyToolModule`）；`computeBoundedEditDistance (Qat)`；`hash differs — job may have been recreated`（`matchNearMissSessionKey`）；`i === o && i.length >= 6`（`matchNearMissSessionKey`）；`F_e(c.filter(p => $w(p.session_key))).slice(0, 3)`（`groupNotifyTargetCandidates`）；`"Background near-miss (NOT user-visible — delivery to these sessions will not reach a user):"`（`renderNotifyTargetCandidateLines`） | confirmed |
| job 会话省略目标时发给 owner | `"A Notify with no target reaches the owner."`（`initManageJobToolModule`）；`d = l ? await nut(n, i) : [await rut(n, e.target_session_key)]`（`runNotifyTool`）；`let r = n.frontmatter.owner_session?.trim();`（`resolveJobOwnerNotifyTarget`） | confirmed |
| correlation_id 与 reply_to 只在 job 与渠道会话提供，渲染成 session-notify 的属性 | `(r === "job" \|\| r === "foreground") && (i.correlation_id`（`buildNotifyInputSchema`）；`"notify_correlation_id"`（`renderMailboxEventPrompt`）；`"notify_reply_to"`（`renderMailboxEventPrompt`） | confirmed |
| 通知链深度上限 5，深度来自本批事件，达到上限即报错 | `M_e = 5`（`initNotifyToolModule`）；`fn = typeof un?.notify_depth == "number" ? un.notify_depth : 0`（`drainSessionMailbox`）；`if (Et = Ve.maxNotifyDepth, Ve.eventIds)`（`createSessionManager`）；`if (a >= M_e) throw new Error(`（`runNotifyTool`）；`likely a loop`（`runNotifyTool`） | confirmed |
| 深度在构造工具集时按值复制，多数路径不逐批更新；pi 回调不传深度 | `notifyDepth: t.notifyDepth,`（`createAladuoMcpServer`）；`E = l.mcpServersFactory ? l.mcpServersFactory() : l.mcpServers`（`createClaudeStreamingSessionFactory`）；`notifyDepth: Et,`（`createSessionManager`）；`let a = t.notifyDepth ?? 0;`（`runNotifyTool`） | confirmed（静态阅读）；对真实循环的效果为未证实推测 |
| 投递报告逐个目标写结果，有目标未投成时整份报告作为错误返回 | `i === r.length ? "Notify delivered." : "Notify partially delivered."`（`renderNotifyDeliveryReport`）；`if (g.filter(v => !v.success).length > 0) throw new Error(V_e(i, u, c, g));`（`runNotifyTool`） | confirmed |
...
| when 要求带时区、不接受已过去的时间，两种用法共用一个解析函数 | `parseJobRearmTime (fw)`；`Zone-less datetimes are rejected because scheduling is pinned to UTC.`（`parseJobRearmTime`）；`A re-arm at or before now would be consumed as already spent at finalize, silently ending the loop.`（`parseJobRearmTime`）；`r = fw(t.when, n),`（`initJobManagerModule`）；`A time at or before now is rejected`（`initRemindDuoduoToolModule`） | confirmed |
...
| Codex 与 Grok 只观察 Skip，不中断这一轮 | `l = f => (f === ws && u && (u.skipObserved = !0), u?.turnId)`（`createCodexAppServerAdapter`）；`Jst(ee) && (R = !0)`（`createGrokAcpAdapter`）；`isGrokSkipToolCall (Jst)` | confirmed（静态阅读） |
...
| drain 按 claude、结果自报、Skip 记录时间三种途径判定跳过；pi 适配器自报 skipped | `await FSe(e, t, n.runtime, Nn)`（`drainSessionMailbox`）；`markTurnSkippedFromSkipRecord (FSe)`；`isClaudeRuntimeOrDefault (Yw)`；`return Number.isFinite(o) && o >= n`（`hasSkipRewindRecordSince`）；`A.tool_name === "Skip" && (C.skipObserved = !0)`（`createPiWorkerAdapter`）；`skipped: A.skipObserved`（`createPiWorkerAdapter`） | confirmed（pi 部分为静态阅读） |
...
| 附件按渠道声明的能力检查，失败时告诉用户限制；通过的文件替换同路径同 MIME 的旧项 | `If the tool fails (unsupported MIME type or file too large)`（`initQueueOutboundAttachmentModule`）；`outbound: n.outbound`（`recordChannelCapabilityDeclaration`）；`spt(s?.channel_capabilities?.[a])`（`runQueueOutboundAttachmentTool`）；`if (typeof l == "number" && o.size > l)`（`runQueueOutboundAttachmentTool`）；`.filter(m => Tg.resolve(m.path) !== i \|\| m.mime !== c)`（`runQueueOutboundAttachmentTool`） | confirmed |
...
经 `createSpineEvent (rn)` 封装、再经 `atomicAppendEvent (on)` 写入的事件类型一共 13 种，每种的类型字面量都位于下表所列写入方的函数体内（confirmed，见证据表）：
...
| `agent.result` | `emitDrainOutputRecords ($c)`；`replyToGatewayCommandEvent (LXe)`；`createMetaSession (Wgt)` | drain 的每条输出按投递目标各写一条（一轮的回复、拒绝执行的说明、history-control 命令的结果、drain 失败的错误文字）；网关命令回复；后台分区一次成功运行 |
| `agent.error` | `drainSessionMailbox (KSe)`；`handleDrainError (Xw)`；`createMetaSession (Wgt)` | drain 或分区运行失败 |
| `agent.tool_use` | `createDrainExecutionEventRecorder (ake)`（drain 路径）；`appendPartitionToolEvent (Ugt)`（后台分区） | 引擎报告一次工具调用；drain 路径跳过标为 ephemeral 的调用，后台分区不检查 ephemeral、全部记录 |
...
后台分区的产出复用 `agent.result`，以 `source.kind = "meta"`、`source.name = "subconscious:<分区名>"` 和 `payload.tick_type = "subconscious"` 区分（confirmed）。drain 路径上的 `agent.result` 由 `emitDrainOutputRecords ($c)` 写出：它为每个投递目标先写一条出站记录，再追加一条 `agent.result`。`drainSessionMailbox (KSe)` 用它写一轮的回复、拒绝执行时的说明（如 `runtime_unavailable`）和 history-control 命令的结果；`handleDrainError (Xw)` 在 drain 失败时用它写错误文字，空闲压缩触发的 drain 失败除外（confirmed）。两个执行事件记录函数的差别在于是否检查 ephemeral：`createDrainExecutionEventRecorder (ake)` 只在工具调用没有标为 ephemeral 时写 `agent.tool_use`，工具结果一律写 `agent.tool_result`；`appendPartitionToolEvent (Ugt)` 只按事件类型分支，后台分区的工具调用和结果全部记录（confirmed）。
...
by_id 索引有保留期，只在 daemon 启动时裁剪（confirmed）。`main (Fyt)` 在取得进程写锁、初始化运行目录之后调用裁剪，并在日志里记下 `kept`、`dropped`、`cutoff` 三个数；裁剪出错时释放写锁并中止启动；两次启动之间索引只增不减。裁剪由 `pruneEventIdIndexByRetention (sse)` 完成：保留 `partition` 不早于"当前 UTC 日期减 N 天"的索引行，无法解析的行一并丢弃，只在有行被丢弃时重写索引文件（confirmed）。N 由 `readSpineIndexRetentionDays (ose)` 从环境变量 `ALADUO_SPINE_INDEX_RETENTION_DAYS` 读取：未设置或为空时取默认值 7，不是正整数时记警告并用 7；变量名与默认值定义在事件日志模块初始化器 `initSpineEventLogModule (Gs)` 里（confirmed）。
...
| 事件对象补 id 与 ISO 时间戳；id 是 `evt_` 前缀加随机 UUID | `id: K9e()`（`createSpineEvent`）；`ts: t.toISOString()`（`createSpineEvent`）；`evt_${Z9e.randomUUID()}`（`generateSpineEventId`） | confirmed |
...
| 13 种落库类型：agent.tool_use 与 agent.tool_result | `type: "agent.tool_use"`（`createDrainExecutionEventRecorder`）；`type: "agent.tool_result"`（`createDrainExecutionEventRecorder`）；`type: "agent.tool_use"`（`appendPartitionToolEvent`）；`type: "agent.tool_result"`（`appendPartitionToolEvent`） | confirmed |
| drain 路径只记录非 ephemeral 的工具调用，后台分区不检查 ephemeral；两个记录函数分别由 drain 与分区会话挂接 | `o.type === "tool_use" && !o.ephemeral`（`createDrainExecutionEventRecorder`）；`if (r.type === "tool_use") {`（`appendPartitionToolEvent`）；`ake(e, t, Y.event.session_key ?? t, n.onExecutionEvent, Y.event.id)`（`drainSessionMailbox`）；`z.push(Ugt(t, o, S.name, Cn)`（`createMetaSession`） | confirmed |
| drain 路径的 agent.result：每个投递目标先写出站记录再追加 agent.result；一轮回复、拒绝说明与失败文字都经它写出，空闲压缩的失败除外 | `type: "agent.result"`（`emitDrainOutputRecords`）；`in_reply_to_event_id: n.event.id`（`emitDrainOutputRecords`）；`await Wl(e, f), r.push(f)`（`emitDrainOutputRecords`）；`"outbox_emit_ms", async () => $c(e, t, {`（`drainSessionMailbox`）；`stage: "runtime_unavailable"`（`drainSessionMailbox`）；`"[runner] failed to emit drain-error outbox record"`（`handleDrainError`）；`"[runner] idle-compact drain error — spine only, no channel record"`（`handleDrainError`） | confirmed |
...
| 裁剪规则：按分区日期保留，丢弃无法解析的行，无丢弃时不重写；默认 7 天，由 ALADUO_SPINE_INDEX_RETENTION_DAYS 改，非正整数时警告并用默认值 | `n.getUTCDate() - t.retentionDays`（`pruneEventIdIndexByRetention`）；`typeof d == "string" && d >= i ? s.push(c) : a += 1`（`pruneEventIdIndexByRetention`）；`retentionDays: ose()`（`main`）；`is not a positive integer; using`（`readSpineIndexRetentionDays`）；`pz = 7, rse = "ALADUO_SPINE_INDEX_RETENTION_DAYS"`（`initSpineEventLogModule`） | confirmed；"无丢弃时不重写"是对该函数体的静态阅读：丢弃计数为 0 的分支在写文件之前返回 |
...
渠道消息经 `channel.ingress` 进入，先过三项检查，再由路由规则决定是否需要模型（confirmed）。三项检查按顺序是 WebSocket 身份参数、归档状态、工作目录：目标会话正在归档时返回 `-32011`，工作目录不可用时返回 `-32010` 并附带指引文字；经 HTTP 调用且未给 `source_kind` 时，来源种类记为 `rpc`；`channel.command` 做同样三项检查（confirmed）。身份参数与工作目录两项检查各由一个函数实现（confirmed）。`assertWsChannelIdentityParams (k0e)` 只检查经 WebSocket 的调用：必须给出业务渠道种类 `source_kind`（不能是 `rpc` 或 `ws`）和稳定的 `channel_id`，否则返回 `-32602`。`resolveIngressWorkspace (x0e)` 依次取参数 `cwd_abs`（已标为 deprecated，使用时记警告）、渠道配置的 `new_session_workspace`（不存在时尝试创建，失败则记警告并继续往下）、会话已绑定的目录、daemon 的工作目录（`ALADUO_WORK_DIR`，缺省为进程当前目录）；参数给出的目录或会话已绑定的目录不存在时返回 `-32010`，不再往下尝试；取到的目录除会话已绑定的那一种外，都写回会话状态，成为该会话的绑定目录。
...
drain 在一轮结束时由 `emitDrainOutputRecords ($c)` 为每个投递目标写一条记录：第一个目标的 `policy` 是 `reply_to_origin`，邮箱条目指定了另一个回复会话时是 `reply_override`，其余目标是 `fanout`；只有目标是渠道会话时，`payload.data` 里才带 `turn_meta`（confirmed）。`turn_meta` 是这一轮的统计，由 `drainSessionMailbox (KSe)` 计算：`elapsed_ms`（从 drain 开始到写出站记录）、`total_input_tokens`、`output_tokens`、`cache_hit_rate`、`total_cost_usd`、`model`、`context_used_tokens` 和 `protocol`（`anthropic`、`codex`、`grok`、`pi`）（confirmed）。按协议的类型注释，渠道适配器用这些字段渲染回复卡片的页脚，Codex 不报告成本。

daemon 内的推送由 `createOutboxDeliveryManager (Qgt)` 负责：它监听 `session.output` 总线事件，按会话串行投递给该会话的 WebSocket 订阅者；会话没有订阅者时记录保持原状态；推送失败时把记录标为 `failed` 并把尝试次数加一；仍为 `pending` 的记录和尝试少于 5 次的 `failed` 记录在每次心跳（`cadence.tick`）和 daemon 启动时重试（confirmed）。`job` 与 `meta` 会话的记录不需要渠道：`isJobOrMetaOutboxRecord (Xgt)` 按渠道种类为 `job` 或 `meta`、或会话键以 `job:` 或 `meta:` 开头识别这类记录，推送函数在检查订阅者之前就把它们标为已送达（confirmed）。
...
| 只读集合的六个方法名 | — | 未证实推测：静态阅读所得，集合是入口代码里没有真名的顶层常量，`createDaemon` 中的调用点只引用集合、不含方法名 |
...
| WebSocket 身份参数的细则与工作目录的解析顺序；取到的目录写回会话状态 | `over WebSocket requires params.source_kind`（`assertWsChannelIdentityParams`）；`must be a business channel kind, not transport kind`（`assertWsChannelIdentityParams`）；`over WebSocket requires params.channel_id`（`assertWsChannelIdentityParams`）；`cwd_abs is deprecated`（`resolveIngressWorkspace`）；`created missing new_session_workspace`（`resolveIngressWorkspace`）；`bound session path does not exist`（`resolveIngressWorkspace`）；`created_at: s?.created_at ?? new Date().toISOString()`（`resolveIngressWorkspace`）；`o = e.workDir ?? t.ALADUO_WORK_DIR ?? process.cwd()`（`resolveRuntimePaths`） | confirmed；`-32602` 是身份检查抛出的参数错误类自带的错误码，该类声明在入口代码里、没有真名 |
...
| 各 policy 的取用条件；turn_meta 只附给渠道会话 | `policy: u === 0 ? a ? "reply_override" : "reply_to_origin" : "fanout"`（`emitDrainOutputRecords`）；`d = n.turnMeta !== void 0 && to(l) === "channel"`（`emitDrainOutputRecords`）；`"outbox_emit_ms"`（`drainSessionMailbox`） | confirmed |
| daemon 内推送：按会话串行，无订阅者时不动，失败标 failed，少于 5 次的在心跳与启动时重试 | `maxAttempts: i = 5`（`createOutboxDeliveryManager`）；`if (r.getSubscribers(h.session_key).length === 0) return !1`（`createOutboxDeliveryManager`）；`error: "delivery failed"`（`createOutboxDeliveryManager`）；`attempts: t.attempts + 1`（`recordOutboxDeliveryAttempt`）；`i.status === "failed" && i.attempts >= t`（`listRetryableOutboxRecords`）；`n.on("cadence.tick", c)`（`createOutboxDeliveryManager`）；`"[pid0] outbox initial flush error"`（`main`） | confirmed |
| job 与 meta 会话的记录在检查订阅者之前直接标为已送达 | `e.channel_kind === "job" \|\| e.channel_kind === "meta"`（`isJobOrMetaOutboxRecord`）；`e.session_key.startsWith("job:") \|\| e.session_key.startsWith("meta:")`（`isJobOrMetaOutboxRecord`）；`if (Xgt(h)) {`（`createOutboxDeliveryManager`） | confirmed |
...
合并成立时，`prepareDrainTurnContext (SH)` 把窗口里的所有事件拼成一段 prompt，每轮瞬时块只生成一次，系统提示与 SDK 配置取自窗口最后一个事件（称为锚点事件）所在渠道的配置；回复只写一次，挂在锚点事件上，窗口里其余事件关联到同一条回复记录（confirmed）。拼接文本由 `renderCoalescedDrainPrompt (yft)` 生成：开头说明这 N 个时间相近的事件作为一次连续更新处理、按从旧到新排列、只回复一次，然后逐个列出每个事件的 `@evt(id)`、类型、时间和正文，正文为空时写 `(empty)`（confirmed）。遥测事件 `sdk_start` 用 `coalesced: pt.length > 1`（`drainSessionMailbox`）标出这是否是一次合并调用。合并不成立时，每个事件各自生成瞬时块、各自解析渠道配置、各自调用一次引擎；`/compact` 这类 history-control 命令也在这条逐条路径上处理（3.6）。
...
adapter 之外还有一层调用包装 `runDrainTurnWithResumeFallback (jft)`，由 `runDrainQueryAndCollectOutboundAttachments (HSe)` 调用：带 resume 的调用失败时，包装层重新生成 prompt、去掉 resume 再调用一次，并在结果上标记 `usedFallback` 与 `resumeError`；abort、turn 中断、prompt 未被接纳这三类错误不重试，直接抛出（confirmed）。drain 收到带这个标记的结果时只追加一条 `agent.error`（7.7，confirmed）。
...
除 `sdk_turn` 外，交给它的阶段还有非渠道会话的 `workspace_unavailable`、`runtime_unavailable`、`runtime_mismatch`，这三种由调用方提供各自的说明文字作为回复（confirmed）。渠道会话遇到这三种情况时不经过它：说明文字作为普通回复写出，事件标记为已处理，drain 返回拒绝阶段（3.3）；其中来源为空闲压缩的事件仍交给它，只写事件日志（`refusedStage: Y`（`drainSessionMailbox`））。另一个阶段是 `context_profile`：drain 在调用引擎之前，先由 `resolveDrainContextProfileOrRefuse (jSe)` 解析 Claude 的模型上下文配置，即配置层里的 `claude.model_profiles` 条目（3.4），解析结果作为调用参数传给引擎（confirmed）。解析失败时，它以阶段 `context_profile` 调用 `handleDrainError (Xw)`，随后重新抛出原错误，turn 不会开始；它提供的回复文字说明 turn 在开始前被拒绝，附上原错误信息（其中写明出错的配置层），并列出三个配置层对应的文件：global 为 `kernel/config/runtime.md`，kind 为 `kernel/config/<kind>.md`，instance 为渠道描述文件或 job 文件（confirmed，`stage: "context_profile"`（`resolveDrainContextProfileOrRefuse`））。
...
| 合并 prompt 的文字格式（事件数、从旧到新、"只回复一次"、逐条 `@evt`、空正文写 `(empty)`） | `renderCoalescedDrainPrompt (yft)`；`b = yft(r)`（`prepareDrainTurnContext`）；`"Events are ordered from oldest to newest. Reply once."`（`renderCoalescedDrainPrompt`）；`t.push(r.prompt || "(empty)")`（`renderCoalescedDrainPrompt`） | confirmed |
...
| 引擎调用经过一层包装函数 | `runDrainTurnWithResumeFallback (jft)`；`let i = await jft(e, t, n, r)`（`runDrainQueryAndCollectOutboundAttachments`） | confirmed |
| 包装函数在 resume 调用失败时重新生成 prompt、去掉 resume 重试一次，abort、中断、未接纳三类错误除外，结果带 usedFallback 与 resumeError | `if (Wh($) || Gv($) || Kv($)) throw $; if (y && (y.flush()`（`runDrainTurnWithResumeFallback`）；`F = await n.run(S(C));`（`runDrainTurnWithResumeFallback`）；`resumeError: $ instanceof Error ? $.message : String($)`（`runDrainTurnWithResumeFallback`） | confirmed |
...
| drain 在调用引擎之前解析模型上下文配置，结果作为调用参数 | `resolveDrainContextProfileOrRefuse (jSe)`；`Ro = await jSe(e, t, {`（`drainSessionMailbox`）；`claudeContextRequirement: Ro.requirement`（`drainSessionMailbox`） | confirmed |
| 解析失败以 context_profile 阶段交给 handleDrainError 并重新抛出，turn 不开始；回复文字列出三个配置层对应的文件 | `throw await Xw(e, t, { anchor: r.anchor`（`resolveDrainContextProfileOrRefuse`）；`stage: "context_profile"`（`resolveDrainContextProfileOrRefuse`）；`kernel/config/<kind>.md`（`resolveDrainContextProfileOrRefuse`） | confirmed |
...
| `mailbox/pending/*.item.json` | 合并后的邮箱项：指针行、事件 id、回复目标会话、`created_at`；标记完成时删除 | `mergeInboxIntoMailbox (fR)` 写入，`deleteMailboxPendingItemsByEventIds (Ao)` 删除 |
| `mailbox/notes.jsonl` | drain 追加的处理说明（如处理数、跳过数、孤项清理数） | `appendSessionMailboxNote (cb)`，由 `drainSessionMailbox (KSe)` 调用 |
...
按前缀分类会话键的函数不止一个；下表列出其中三个，它们对 `cadence:` 与 `subconscious:` 两个前缀的归类并不一致（confirmed）：
...
三个函数服务不同的调用点（confirmed）。第一列的函数用于对外 RPC 的隔离检查和用户可见会话的筛选（8.5）；第二列的函数用于 drain 内只对渠道会话生效的处理，例如引擎被拒时把说明作为普通回复写出（3.3）；第三列的函数在 `state.json` 没有记录平面时提供默认值，网关侧另有一个规则相同的函数 `classifySessionPlaneByKey (hyt)`。并发池的归属由另一个函数决定（8.3）。

actor 记录按会话键存放，每次启动 actor 时生成一个新对象替换旧记录，`actorRunId` 单调递增，并从旧记录继承 SDK 会话 id、常驻连接的代数、渠道附着集合、来源、job id、引擎和 adapter（confirmed，`"[session-manager] actor start"`（`createSessionManager`））。唤醒启动 actor 时，唤醒函数用 `inferActorOriginFromSessionKey (GW)` 按前缀推断来源：`job:` 为 job，`meta:`、`cadence:`、`system:` 为 system，其他前缀推断不出；推断不出时沿用旧记录的来源，没有旧记录则为 channel（confirmed，`origin: P?.origin ?? K?.origin ?? "channel"`（`createSessionManager`））。
...
drain 租约文件是 `run/locks/<会话键的 sha256>.json`，由 `drainSessionMailbox (KSe)` 在每次执行开始时用 `acquireSessionDrainLock (gSe)` 取得、结束时用 `releaseSessionDrainLock (_Se)` 删除（confirmed）。租约记录会话键哈希、pid 与心跳时间；已有租约且未过期（默认 TTL 120 秒）时取不到，drain 直接返回，结果里 `lockAcquired` 为假；执行期间每隔 `ALADUO_SESSION_HEARTBEAT_MS`（默认 30 秒）刷新心跳。在同一个 daemon 内，一个会话键只有一个 actor、一个 drain 循环，正常运行时租约总能取到；它具体防范哪种并发，代码里没有说明，属未证实推测。
...
归档只在会话没有待处理工作时发生（confirmed）。`archiveSessionAndArtifacts (Ybe)` 在按键互斥内再检查一次 inbox 与邮箱：有待处理项时返回 `pending_work`，读不出时返回 `unreadable`，两种情况都不移动任何文件；检查通过后把会话目录、原始请求快照目录、出站 replay 日志、该会话的出站记录和它绑定的渠道目录（如有）分别移到各自的归档目录。`session.archive` 的处理函数 `archiveSessionIfQuiescent (byt)` 先设置归档标记，标记已存在（另一次归档正在进行）时拒绝；随后拒绝有未结束 actor、有排队唤醒、inbox 或邮箱非空的会话，检查都通过才调用 `archiveSessionAndArtifacts`，并把它返回的 `pending_work` 与 `unreadable` 同样作为拒绝；所有拒绝都返回 `reason: "active"` 和一条说明如何处理的 `hint`（confirmed）。处理函数结束时清除标记；没有归档成功且会话仍有待处理项时，以 `never` 档位重新唤醒这个会话（confirmed，`"[session.archive] post-refusal wake re-dispatch failed"`（`archiveSessionIfQuiescent`））。
...
空闲会话还有一个独立的机制：空闲压缩（confirmed）。`createIdleCompactSweeper (Xbe)` 定期扫描会话索引里的渠道会话，每次扫描至多触发 8 次（`IDLE_COMPACT_FIRE_CAP_PER_SWEEP (jut)`）；一个会话同时满足以下条件才会被压缩：
...
| 会话目录内的文件布局 | `Hr.join(Jn(e, t), "inbox")`（`resolveSessionInboxDir`）；`_${gse()}.pending`（`enqueueSessionInboxLine`）；`"quarantine"`（`mergeInboxIntoMailbox`）；`Hr.join(Jn(e, t), "mailbox", "pending")`（`resolveSessionMailboxPendingDir`）；`.item.json`（`mergeInboxIntoMailbox`）；`Hr.join(Jn(e, t), "mailbox", "notes.jsonl")`（`resolveSessionMailboxNotesPath`）；`Hr.join(Jn(e, t), "mailbox.md")`（`resolveSessionMailboxMarkdownPath`）；`"# Session Mailbox"`（`renderSessionMailboxFile`）；`Hr.join(Jn(e, t), "meta.md")`（`resolveSessionMetaPath`）；`"# Session Descriptor"`（`ensureSessionDescriptorAndStateFiles`）；`Hr.join(Jn(e, t), "state.json")`（`resolveSessionStatePath`） | confirmed |
| drain 追加处理说明到 notes.jsonl | `note: n.trim()`（`appendSessionMailboxNote`）；`orphan_cleanup=`（`drainSessionMailbox`） | confirmed |
| 标记完成即删除对应的 `.item.json` 文件 | `if (!a.endsWith(".item.json")) continue;`（`deleteMailboxPendingItemsByEventIds`）；`await yr.unlink(Ys.join(r, a))`（`deleteMailboxPendingItemsByEventIds`）；`"[runner] eager markDone failed (will retry at drain end)"`（`drainSessionMailbox`） | confirmed |
...
| 表中三个前缀分类函数的规则 | `e.startsWith("subconscious:") ? "subconscious"`（`classifySessionKeyKind`）；`e.includes(":") ? "channel" : "unknown"`（`classifySessionKeyOrUnknown`）；`e.startsWith("cadence:") ? "system" : "work"`（`classifySessionPlane`） | confirmed（bundle 里另有多个没有真名的同类函数） |
...
| 缺少来源时按前缀推断：`job:` 为 job，`meta:`、`cadence:`、`system:` 为 system；推断不出时沿用旧记录，没有旧记录为 channel | `if (e.startsWith("job:")) return {`（`inferActorOriginFromSessionKey`）；`if (e.startsWith("system:")) return {`（`inferActorOriginFromSessionKey`）；`e.startsWith("cadence:")`（`inferActorOriginFromSessionKey`）；`"[session-manager] wake starting actor with inferred origin"`（`createSessionManager`）；`origin: P?.origin ?? K?.origin ?? "channel"`（`createSessionManager`） | confirmed |
...
| drain 租约文件的获取、心跳与释放 | `acquireSessionDrainLock (gSe)`；`session_ref: t`（`acquireSessionDrainLock`）；`o = n.ttlMs ?? 12e4`（`acquireSessionDrainLock`）；`if (a && !jdt(a, i, o)) return {`（`acquireSessionDrainLock`）；`refreshSessionDrainLockHeartbeat (ySe)`；`releaseSessionDrainLock (_Se)`；`if (!(await gSe(e, r)).acquired) return {`（`drainSessionMailbox`）；`let o = n.lockHeartbeatIntervalMs ?? 3e4`（`drainSessionMailbox`）；`clearInterval(s), await _Se(e, r)`（`drainSessionMailbox`）；`lockHeartbeatIntervalMs: o`（`createSessionManager`）；`heartbeatIntervalMs: Number(process.env.ALADUO_SESSION_HEARTBEAT_MS ?? 3e4)`（`main`） | confirmed |
...
| 插话回调把在途事件当作已处理并标记完成 | `if (w.inflightEventIds.has(yt.eventId)) {`（`createSessionManager`）；`cr.length > 0 && await Ao(t, P, cr)`（`createSessionManager`）；`deleteMailboxPendingItemsByEventIds (Ao)`；`processedEventIds: [...cr]`（`createSessionManager`） | confirmed |
...
| 经 WebSocket 的 ingress 与 command 必须带业务 source_kind 与 channel_id；HTTP 上缺省记为 rpc | `if (!n?.wsSubscriberId) return;`（`assertWsChannelIdentityParams`）；`must be a business channel kind, not transport kind`（`assertWsChannelIdentityParams`）；`over WebSocket requires params.channel_id`（`assertWsChannelIdentityParams`）；`k0e("channel.ingress", k, D)`（`createDaemon`）；`k0e("channel.command", k, D)`（`createDaemon`）；`N = k.source_kind ?? (D?.wsSubscriberId ? "ws" : "rpc")`（`createDaemon`） | confirmed（校验只在带 wsSubscriberId 时进行，缺 source_kind 即抛错，所以 `createDaemon` 里缺省取 "ws" 的分支执行不到） |
...
| 带 cwd_abs 的 ingress 记弃用警告，目录存在时以它为会话工作目录，不存在时拒绝 | `cwdAbs: k.cwd_abs`（`createDaemon`）；`cwd_abs is deprecated; prefer descriptor-driven new_session_workspace`（`resolveIngressWorkspace`）；`Use an existing absolute directory path and retry.`（`resolveIngressWorkspace`） | confirmed |
...
调度规则有五种写法，全部按 UTC 计算，到期判断由 `isJobScheduleDue (Iye)` 完成：
...
下文的"一次性调度"指 once、@in、keepalive 三种：扫描器的重试规则（10.2）和投递负载里的调度类型 one-shot 都按这三种判断（confirmed，`isOneShotJobSchedule (r$)`）；结算后自动归档只适用于 once 与 @in（confirmed，见证据表）。无论哪种规则，状态文件里的 run_at 一旦到达，job 就到期一次，这是改期的实现方式（10.4）；对还没被认领过的 `once` 与 `@in`，一个尚未到达的 run_at 同时推迟它的首次运行（confirmed，`if ((e === "once" || e.startsWith("@in ")) && t === null) return !1`（`isJobScheduleDue`））。
...
每次扫描在检查 job 之前，先由 `fireDueWakeRecords (Ggt)` 处理到期的提醒记录：把记录里的上下文作为 self-wake 事件投递给它的 owner 会话，然后归档这条记录，投递返回失败时同样归档，投递过程抛出异常时记录留到下一次扫描（`return await this.archiveJobHoldingLock(t), a.success ? {`（`initJobManagerModule`），细节见 4.4）。
...
| 五种调度规则的首次到期、此后到期与自动归档 | `if (e === "once" \|\| e === "keepalive") return t !== null ? !1 : s ? n.getTime() >= o : !0`（`isJobScheduleDue`）；`return t !== null ? !1 : s ? n.getTime() - o >= u : !0`（`isJobScheduleDue`）；`return n.getTime() - l >= u`（`isJobScheduleDue`）；`currentDate: new Date(t),`（`isJobScheduleDue`）；`let l = u.state.last_scheduled_at ?? u.state.last_run_at`（`scanAndSpawnDueJobs`）；`'once' — run once on next scheduler cycle, then auto-archive (session is archived with it)`（`initManageJobToolModule`）；`'keepalive' — run once on next scheduler cycle, then stay dormant.`（`initManageJobToolModule`）；`Never auto-archives`（`initManageJobToolModule`） | confirmed（`isJobScheduleDue` 中 t 是上一次认领时间，没有时取 last_run_at，o 是创建时间：once、keepalive 与 @in 认领过一次后不再按规则到期，`@every` 从上一次认领时间起算，cron 取上一次认领时间之后的下一个边界；自动归档的文字来自 ManageJob 工具说明） |
| 到期判断含 run_at：run_at 到达即到期一次；还没被认领过的 once 与 @in 在 run_at 之前不到期 | `if (n.getTime() >= a) return !0;`（`isJobScheduleDue`）；`if ((e === "once" \|\| e.startsWith("@in ")) && t === null) return !1`（`isJobScheduleDue`）；`!Iye(u.frontmatter.cron, l, o, u.frontmatter.created_at, u.state.run_at ?? null)`（`scanAndSpawnDueJobs`）；`Set ONE extra fire at <when>`（`cli:jobHelp`） | confirmed |
...
| 一次性调度认领后未开始的运行被重新判为到期；一次性调度指 once、@in、keepalive，投递负载据此写 one-shot 或 periodic | `r$(u.frontmatter.cron) && u.state.last_scheduled_at && f`（`scanAndSpawnDueJobs`）；`u.state.last_run_started_at ? new Date(u.state.last_run_started_at).getTime() : Number.NaN`（`scanAndSpawnDueJobs`）；`return e === "once" \|\| e.startsWith("@in ") \|\| e === "keepalive"`（`isOneShotJobSchedule`）；`return r$(e) ? "one-shot" : "periodic"`（`classifyJobScheduleType`）；`schedule_type: AS(x)`（`createJobSessionFinalizer`） | confirmed |
| 扫描先投递到期提醒，再检查 job；提醒作为 self-wake 事件投递给 owner；投递返回成功或失败都归档提醒记录，抛出异常时留到下一次扫描 | `a = await Ggt(e, r, o, n?.bus)`（`scanAndSpawnDueJobs`）；`eventType: "self-wake"`（`fireDueWakeRecords`）；`targetSessionKey: c.frontmatter.owner_session`（`fireDueWakeRecords`）；`"[cadence] wake fire failed; record left for the next scan"`（`fireDueWakeRecords`）；`return await this.archiveJobHoldingLock(t), a.success ? {`（`initJobManagerModule`）；`outcome: "undeliverable",`（`initJobManagerModule`） | confirmed（`scanAndSpawnDueJobs` 在遍历 job 的循环之前调用 `fireDueWakeRecords`） |
...
| once 与 @in 结算后自动归档，改期或运行中归档时跳过；keepalive 与周期调度不自动归档 | `return e === "once" \|\| e.startsWith("@in ")`（`isAutoArchivedJobSchedule`）；`if (!_Ee(h)) return !1`（`createJobSessionFinalizer`）；`"[session-manager] auto-archived one-shot job"`（`createJobSessionFinalizer`）；`"[session-manager] skip auto-archive: job re-armed via reschedule"`（`createJobSessionFinalizer`）；`"[session-manager] skip auto-archive: job already gone (archived mid-run)"`（`createJobSessionFinalizer`） | confirmed |
| 被中止的 once 或 @in job 归档时才通知 owner；被重启切断、之后还会再运行的一次不通知 | `"[session-manager] job run cancelled after turn ack — archived, owner notified"`（`createJobSessionFinalizer`）；`"[session-manager] job run cancelled after turn ack — consumed + failure marker, job preserved, no delivery"`（`createJobSessionFinalizer`）；`isAutoArchivedJobSchedule (_Ee)`；`A failed run wakes you; a run cut off by a restart that will run again does not.`（`initManageJobToolModule`） | confirmed（CANCELLED_POST_ACK 分支只在自动归档返回真时发出 job.fail，而自动归档只对 once 与以 "@in " 开头的规则进行） |
...
**活动指纹。**指纹有四个分量：memory 下 fragments、entities、topics 三个目录里最新的修改时间（`readNewestMtimeRecursive (FA)` 递归取目录和文件 mtime 的最大值），以及 `readLatestExternalEventId (Bgt)` 在最新一天的事件日志文件里从后往前找到的第一条外部事件的 id。"外部"指事件的 `source.kind` 不属于 `cadence`、`meta`、`system`、`runner`、`route`、`gateway` 这六个内部来源，所以渠道消息、job 事件（来源 `job`）和配置变更事件（来源 `rpc`）都算外部活动。四个分量以冒号拼接，由 `hashActivityFingerprint (qgt)` 取 sha256 的前 16 个十六进制字符；与上一次心跳的值相同就跳过整次心跳（confirmed）。
...
**初始化顺序。**初始化函数依次：把旧格式的会话注册目录 `var/registry/sessions/` 补写进各会话状态文件后归档（`archiveLegacyRegistrySessionsDir (iSe)`）；创建运行时目录（包括 `var/cadence/`），把 `run/` 目录权限设为 0700；迁移旧格式的 job 会话键；从出厂目录复制文件到内核；把出厂目录 `var/` 下（含顶层和各级子目录）的每个 `DUODUO.md` 复制到 `~/.aladuo/var/` 的对应位置，每次覆盖（`refreshBootstrapDuoduoMdFiles (xdt)`）；创建 memory 与 subconscious 的各级目录、分区状态目录和内核的 `.claude/` 目录；轮转表与记忆板不存在时分别写入空模板和空文件；退休分区；确保内核是 git 仓库并同步 `.gitignore`；为每个分区编译 Codex 子代理定义；最后在 daemon 状态文件不存在时写入初始状态（confirmed；调用顺序见 `initializeRuntime (Rdt)`，每一步的实现函数列在证据表）。

**只补缺失。**出厂目录默认是 npm 包内的 `bootstrap/`，可用 `ALADUO_BOOTSTRAP_DIR` 改（`resolveRuntimePaths (Yut)`）。复制由 `copyBootstrapIntoKernel (Edt)` 完成：出厂目录不存在、或内核目录与出厂目录是同一路径时不复制；复制时跳过出厂目录顶层的 `var/` 以及 `meta-prompt.md`、`dashboard.html` 两个文件，面板页面每次从出厂目录读取，身份提示按第 2 节的规则经环境变量定位，两者都不进内核。内核目录为空时整棵复制；内核已有内容时递归复制，目标文件已存在就跳过。只有 `NODE_ENV=development` 时规则不同：`memory/` 与 `config/` 仍只补缺失，其余目录和顶层文件每次覆盖复制。因此 npm 升级不会覆盖内核里已有的分区提示词、记忆板或配置，上游对分区提示词的修改需要人工执行 `skills/duoduo-runtime-admin` 技能里的 subconscious-refresh 流程才能进入内核（confirmed；上游技能 `skills/duoduo-runtime-admin/SKILL.md` 原文同样写明 install merges missing files only）。

**.gitignore。**`ensureKernelGitRepo (Uwe)` 先判断内核目录是不是某个 git 仓库的顶层（`git rev-parse --show-toplevel` 的输出与内核目录的真实路径相同）。不是时执行 `git init`，写入 `.gitignore` 模板，`git add .` 后以 "memory: genesis" 为说明提交第一个 commit；已经是仓库时，由 `mergeKernelGitignoreEntries (odt)` 把模板中非注释、非空的行里 `.gitignore` 还没有的逐行追加到末尾，不删除、不改动已有的行，没有要追加的行时不写文件。模板定义在 `initKernelGitModule (qwe)` 里，条目是 `memory/state/`、`memory/fragments/`、`memory/effectiveness/`、`subconscious/inbox/`、`.claude/`、`*.tmp`、`CLAUDE.local.md`、`config/runtime.md`，第一行注释是 "# Runtime state (not part of cognitive evolution)"，`config/runtime.md` 前另有一行注释说明模型配置可能含有凭据形态的值（confirmed）。
...
| 系统提示按引擎分：Grok 另生成并使用 `prompt_mode`；Claude 不传、由适配器按环境变量补预设与身份提示；Codex 适配器不带会话配置，developer instructions 只是列出动态工具名的提醒；pi 不传，pi 进程收到空的 `system_prompt` | `prompt_mode: S.prompt_mode ?? "append"`（`createMetaSession`）；`systemPrompt: G`（`createMetaSession`）；`preset: "claude_code"`（`createAgentSdkAdapter`）；`a = e.codexAdapterFactory ?? (() => yw({`（`createMetaSession`）；`y && (H.developerInstructions = y)`（`createCodexAppServerAdapter`）；`"<duoduo-reminder>"`（`buildDeveloperInstructions`）；`buildPiSystemPromptSpec (qft)`；`system_prompt: l`（`createPiWorkerAdapter`） | confirmed（pi 进程只在 mode 为 "override" 时替换系统提示依据 pi-worker.js 字面量；pi 库的缺省系统提示内容未核对） |
...
| 出厂目录默认是包内 `bootstrap/`；同一路径时不复制；跳过顶层 `var`、`meta-prompt.md`、`dashboard.html`；空内核整棵复制，否则只补缺失；`NODE_ENV=development` 时 memory、config 以外覆盖复制；面板页面从出厂目录读取 | `t.ALADUO_BOOTSTRAP_DIR ?? vt.join(ive, "bootstrap")`（`resolveRuntimePaths`）；`nr.resolve(e.kernelDir)`（`copyBootstrapIntoKernel`）；`iH = new Set(["var"]), oH = new Set(["meta-prompt.md", "dashboard.html"])`（`initRuntimeInitializationModule`）；`await Sdt(e.kernelDir)`（`copyBootstrapIntoKernel`）；`copyDirTreeMissingOnly (aH)`；`t.NODE_ENV === "development"`（`copyBootstrapIntoKernel`）；`kdt = new Set(["memory", "config"])`（`initRuntimeInitializationModule`）；`oo.join(u.bootstrapDir, "dashboard.html")`（`createDaemon`） | confirmed（"只补缺失"另有上游技能原文支撑） |
| 内核不是 git 仓库顶层时 `git init`、写模板并提交 genesis；已是仓库时只追加模板里缺失的非注释行 | `"--show-toplevel"`（`isKernelGitToplevel`）；`"memory: genesis"`（`ensureKernelGitRepo`）；`"[memory-git] merged .gitignore entries"`（`mergeKernelGitignoreEntries`）；`!e.startsWith("#")`（`initKernelGitModule`）；`Model-profile config can embed credential-shaped values`（`initKernelGitModule`） | confirmed |
...
**执行顺序。**检查按固定顺序运行各子步，每一步都包在 `runMemoryCheckSubStep (H6)` 的 try/catch 里，一步失败不影响其余各步；外层 `runReadAuditedMemoryCheckStep (ua)` 另外统计这一步有没有遇到读不出（不是"不存在"）的路径。顺序是：`orphan-states` 先算出孤儿状态；契约过滤至少放行一类信号时，再依次运行 board-lint、entity-lint、node-lint、gap-lint、fold-lint、broadcast-budget、broadcast-lint、broadcast-flatten、activation-lint、orphan-newborn-island；这些步全部成功、没有读不出的路径、并且孤儿状态算出来了，才运行最后的 `inbox-sync`；activation-lint 算出的比值非空时，`recordTelemetryMetric (ps)` 把它写成一条名为 `memory_activation_loss` 的指标记录，以一行 JSON 追加到运行时目录 `var/telemetry/` 下按记录时间戳选定的文件，写入失败被忽略；环境变量 `ALADUO_TELEMETRY_ENABLED` 设为 `0`、`false`、`no` 或 `off` 时不写，未设置时写；遗忘开关打开时最后运行 `orphan-forget`（12.3）（confirmed；文件名是记录时间的 UTC 日期加 `.jsonl` 这一点为未证实推测，生成文件名的函数仍没有真名）。
...
board-lint、entity-lint、node-lint 用常量 `B6 = 1`（`initMemoryCheckTickModule`）作为每类上限，按各自的排序只取最差的一条；其余各类的任务单文件名固定，每类每次心跳至多一张；只有 orphan-newborn 每个节点一张（confirmed）。entity-lint 排序用的日期戳计数由 `countDatedStampLines (dve)` 算出，它数的是含有 2026 年日期的行数：正则只认 `2026-MM-DD` 与 `2026MMDD` 两种写法，后一种还要求月份在 01 到 09 之间，其他年份的日期不参与排序（confirmed）。

activation report 统计一段时间内前台工具调用读到了哪些 entities、topics 文件：它给出记忆板每一行指向的文件被读过几次、在窗口开始前就存在却一次都没被读过的文件数，以及被读过却不在可达集里的文件（最多列 10 个）；前台对记忆目录的写入单独计数，不算作"被读过"（confirmed）。窗口由 `scanActivationWindowTouches (fct)` 计算：从今天（UTC 日期）往前逐日读取事件日志，含有 `channel.message` 事件的日子算一个交互日，累计满 30 个交互日为止，窗口内没有消息的日子同样读入；"前台工具调用"指同时含 `"agent.tool_use"` 与 `"kind":"runner"` 的日志行，即来源为 runner 的工具调用事件，"读到"指该事件的参数摘要 `input_summary` 里出现 memory 目录下 entities 或 topics 文件的绝对路径（confirmed）。

board-lint 的"已裁定删除"来自效果记录里的判定词：`buildBoardLintTarget (glt)` 调用 `parseUpdaterGuidanceVerdict (ult)`，后者只在 `updater guidance:` 这一行之后的 `- ` 列表项里找 PRESERVE、KEEP、REMOVE、REWRITE、SHARPEN、DROP；而 intuition-weaver 提示词给出的效果记录模板用的小节名是 `Board guidance:`。按出厂模板写出的效果记录，判定词解析结果为空，"变弱且已裁定删除的不发"这条排除不会生效，变弱的旧格式节点仍会收到 revise 任务单（confirmed，由解析函数与模板原文推出；实际写出的效果记录是否都按模板，未在运行中的实例上观测）。
...
2. 已交出的时间段记在运行时目录的 `var/memory/gap-handed-days.txt`，每行是一整天或 `日期[起,止]` 形式的区间（毫秒精度，UTC）。文件不存在时，`readOrSeedGapHandedDays (Hlt)` 把 `memory/fragments/` 下已有的日期目录名逐行写进这个文件，也就是把这些日子当作整天已交出；之后每交出一个区间就追加一行（confirmed）。
...
| 子步顺序与 activation 指标记录 | `ua("orphan-states"`（`runMemoryCheckTick`）；`ua("orphan-newborn-island"`（`runMemoryCheckTick`）；`"memory_activation_loss"`（`runMemoryCheckTick`） | confirmed |
| 指标以一行 JSON 追加到 `var/telemetry` 下按时间戳选定的文件；`ALADUO_TELEMETRY_ENABLED` 未设置时写，设为关闭值时不写；写入失败被忽略 | `recordTelemetryMetric (ps)`；`appendTelemetryRecord (k5e)`；`isTelemetryEnabled (x5e)`；`if (x5e()) try`（`recordTelemetryMetric`）；`bz(e.ALADUO_TELEMETRY_ENABLED) ?? !0`（`isTelemetryEnabled`）；`b5e.appendFile(S5e(e, t.ts)`（`appendTelemetryRecord`）；`b = vt.join(u, "telemetry")`（`resolveRuntimePaths`） | confirmed（文件名的日期格式为未证实推测：生成文件名的函数仍没有真名） |
...
| board-lint 只看记忆板链接到的 topics 节点；revise 给 pattern-tracker、merge 给 intuition-weaver | `collectBoardLintReport (yve)`；`runBoardLint (ylt)`；`Ic(l) && o.push(glt(u, i, r, l))`（`collectBoardLintReport`）；`buildBoardLintTarget (glt)`；`partition: "pattern-tracker"`（`runBoardLint`）；`merge-${s.slug}.md.pending`（`runBoardLint`） | confirmed |
| revise 的过滤条件：有效果记录、行为类、旧格式，排除变弱且判定为 REMOVE 或 DROP 的节点 | `s.trajectory !== "NO-EFF" && s.cls === "behavioral" && s.fmt === "legacy"`（`runBoardLint`）；`n[1] === "Pattern" ? "legacy"`（`classifyTopicNodeFormat`）；`classifyTopicNodeType (clt)`；`s.trajectory === "WEAKENING" && (s.verdict === "REMOVE" \|\| s.verdict === "DROP")`（`runBoardLint`） | confirmed |
| 效果记录判定词只在 `updater guidance:` 小节下解析，与 intuition-weaver 模板的 `Board guidance:` 不一致 | `parseUpdaterGuidanceVerdict (ult)`；`/updater guidance:/i.test(n)`（`parseUpdaterGuidanceVerdict`）；`olt = ["PRESERVE", "KEEP", "REMOVE", "REWRITE", "SHARPEN", "DROP"]`（`initBoardLintModule`）；`f = ult(a)`（`buildBoardLintTarget`） | confirmed（模板见 `subconscious/intuition-weaver/CLAUDE.md` 的 Effectiveness Files 一节；排除不生效是由两者推出的结论，未在运行中的实例上观测） |
| entity-lint 的三个收敛小节、排序分数与目标分区 | `runEntityLint (Sve)`；`wve = ["What it is now", "Relationship", "Open variables"]`（`initEntityLintModule`）；`m = d * 1e3 + p`（`runEntityLint`）；`entity-converge-${l.slug}.md.pending`（`runEntityLint`）；`countDatedStampLines (dve)`；`for (let r of Bo(e)) t.test(r) && n++`（`countDatedStampLines`） | confirmed |
| node-lint 允许的小节、行数上限与 WASTED-COMPUTE 标注 | `runNodeLint (xve)`；`t === "groove" && e === "References"`（`isAllowedNodeSection`）；`if (p.length === 0 && m <= E6) continue`（`runNodeLint`）；`escalated: WASTED-COMPUTE`（`renderNodeConvergeSignalBody`）；`E6 = 200`（`initNodeLintModule`） | confirmed |
...
| activation report：30 个交互日窗口，热孤儿最多列 10 个，前台写入工具单独计数 | `runActivationLint (rwe)`；`renderActivationReportBody (gct)`；`L6 = 30, nwe = 10`（`initActivationLintModule`）；`"NotebookEdit"`（`initActivationLintModule`） | confirmed |
| 窗口从今天往前逐日读，满 30 个含 `channel.message` 的日子为止；前台工具调用与"读到"的判定 | `scanActivationWindowTouches (fct)`；`s.interactionDays >= L6`（`scanActivationWindowTouches`）；`p.includes('"channel.message"')`（`scanActivationWindowTouches`）；`!p.includes('"kind":"runner"')`（`scanActivationWindowTouches`）；`m.payload?.input_summary`（`scanActivationWindowTouches`） | confirmed |
...
| 只有外部来源的 `channel.message` 算交互；外部判定用六个内部来源的集合 | `readGapLintDayEvents (Uve)`；`interaction: i.type === Flt`（`readGapLintDayEvents`）；`eO.has(s)`（`readGapLintDayEvents`）；`Flt = "channel.message"`（`initGapLintModule`） | confirmed |
| 区间交出后记入已交清单；无外部事件的区间不发任务单但照样记入；被拒时不记 | `c && zve(r.varDir, u.span)`（`deliverScanGapSignal`）；`d && zve(r.varDir, u.span)`（`deliverScanGapSignal`）；`c = l.posted.length > 0`（`deliverScanGapSignal`）；`appendGapHandedSpan (zve)` | confirmed |
| 已交清单是 `var/memory/gap-handed-days.txt`；文件不存在时把 fragments 下的日期目录名逐行写入 | `readOrSeedGapHandedDays (Hlt)`；`Wve = "gap-handed-days.txt"`（`initGapLintModule`）；`Cc.join(t, "memory", Wve)`（`readOrSeedGapHandedDays`）；`a = Hlt(t, r.varDir, s)`（`deliverScanGapSignal`）；`listMemoryFragmentDates (Ult)`；`r.isDirectory() && I6(r.name)`（`listMemoryFragmentDates`）；`flag: "wx"`（`readOrSeedGapHandedDays`） | confirmed |
...
| 遗忘的命令序列：以 memory 目录为工作目录执行 git rm、git diff、带路径的 commit，失败时 reset 与 checkout；index.lock 存在时放弃；未跟踪的文件不被删除 | `e.filter(a => a.state === "STALE")`（`forgetMemoryEntry`）；`o = Zw.join(t, "memory")`（`forgetMemoryEntry`）；`".git", "index.lock"`（`forgetMemoryEntry`）；`"rm", "--ignore-unmatch", "--", ...i`（`forgetMemoryEntry`）；`"--diff-filter=D"`（`forgetMemoryEntry`）；`"user.name=aladuo"`（`forgetMemoryEntry`）；`"reset", "--quiet"`（`forgetMemoryEntry`）；`s = Mct(o)`（`forgetMemoryEntry`）；`resolveGitToplevelSync (Mct)` | confirmed |
| 默认布局下遗忘的 commit 与回滚因路径基准不同而失败，删除停在暂存区；批次中有本地修改时 git rm 整体失败 | `"--name-only"`（`forgetMemoryEntry`）；`cwd: o`（`forgetMemoryEntry`）；`await Uwe(e.kernelDir)`（`initializeRuntime`）；`ensureKernelGitRepo (Uwe)`；`await tH(e, ["init"])`（`ensureKernelGitRepo`）；`"--show-toplevel"`（`isKernelGitToplevel`） | 未证实推测（按 git 默认路径规则推出，并在本地 git 2.50 的临时仓库上复现：diff 输出 `memory/topics/x.md`，commit 与 checkout 报 pathspec 不匹配，状态保持 `D  topics/x.md`；对有本地修改的文件 git rm 报 has local modifications；未在运行中的 daemon 上观测） |
...
| 两个分区空输出判为 invalid_output | `"(no output)"`（`detectEmptyRequiredPartitionOutput`）；`Fgt = new Set(["gradient-distiller", "intuition-weaver"])`（`initMetaSessionModule`） | confirmed |
...
**记忆板以外的指令变化，Claude。** 会话管理器发出 `session.streaming_invalidated`，原因为 `instructions_drift`。事件处理函数把该 actor 的流式适配器置空，并把存活流式进程的 `needsRecreation` 置为真；下一个 turn 里 `createClaudeStreamingSessionFactory (s0e)` 的复用条件因此不成立，先拆除旧进程，再以会话状态中的同一个会话 id 启动新进程，新进程收到当前算出的系统提示（confirmed）。对话历史因为续接原会话而保留。新指令能在续接的会话上生效，依赖 2.4 所述的"关闭 SDK 系统提示快照"：在系统提示记录功能生效的环境里，不关闭快照时 SDK 会一直沿用会话第一次请求时记录的系统提示（2.4，SDK 类型文档原文）；运行时在构造 SDK 选项时由 `disableSystemPromptSnapshot (Prt)` 把每个系统提示都设为 `snapshot: false`，一次性调用与常驻流式会话都经过这一步（confirmed）。非渠道 Claude 会话同样每次调用带当前文本，不受这个事件影响。
...
**Grok 与 pi。** 对这两个引擎，`runInstructionsFingerprintGuard (OA)` 在两种变化下都只写入新指纹并记一条"指令已更新"日志，不清空会话、不发事件（confirmed）。新文本何时到达引擎由适配器决定：Grok 的 `override` 文本在下一次调用前重新载入会话，`append` 文本只在新建会话时发送；pi 的系统提示只在 worker 进程启动时生效，要等 worker 重启（例如空闲回收，8.4）才换上新文本（confirmed：daemon 侧的 `createPiWorkerAdapter (TO)` 只在还没有 worker 进程时取当次调用的系统提示，写进 init 帧；pi-worker 也只读取第一个 init 帧里的系统提示）。
...
| pi：系统提示只在没有 worker 进程时取值，随 init 帧发送 | `!r && !i ? (a = k, l = qft($.systemPrompt))`（`createPiWorkerAdapter`）；`system_prompt: l`（`createPiWorkerAdapter`） | confirmed |
...
| 系统提示的种类层与实例层按锚点事件解析，非渠道会话的这两层不进指纹 | `async () => YV(e, u.event)`（`prepareDrainTurnContext`）；`return cbe({`（`resolveEffectiveChannelConfigForEvent`）；`n.origin === "channel"`（`collectInstructionsInputs`） | confirmed |
...
3. **效果记录判定词的小节名不一致（12.2）。** 解析函数 `parseUpdaterGuidanceVerdict (ult)` 只在 `updater guidance:` 小节下找判定词，intuition-weaver 提示词给出的模板用的是 `Board guidance:`，所以"变弱且已裁定删除的节点不发 revise 任务单"这条排除可能从不生效。
...
正文里还有一批未证实推测，代码已在 pretty bundle 读到，缺的只是一个构建检查能核对的引用。除一项外，它们都在 cli bundle 里：cli bundle 只有 `__export` 表给出的真名，还没有推断名映射，这些 CLI 函数因此没有真名。剩下的一项是只读 TCP 端口放行的方法集合，它是 daemon bundle 顶层的一个常量，不在任何已命名的函数或模块初始化器里。给这些代码登记推断名之后，对应主张可以改成按名引用并标 confirmed。按节列出如下：
...
`createDaemon (Lyt)` 的分发函数按方法名逐个比较，识别下面 33 个方法，其余方法返回 `-32601`；三个监听器共用这一个分发函数（6.1）。表中按用途分组，"处理"一栏给出处理函数的真名；处理逻辑直接写在分发函数里的方法标"内联"。
...
| session | `session.list` | `listSessionIndexSummaries (h$)` | 8.1 |
| session | `session.archive` | `archiveSessionIfQuiescent (byt)`，移动文件的是 `archiveSessionAndArtifacts (Ybe)` | 8.2 |
| session | `session.set_alias` | `setSessionAliasAndReindex (wyt)`，写显示名的是 `updateSessionDisplayName (ole)` | 8.1 |
...
| session | `session.config` | `applySessionConfigVerb (Tyt)`，改配置后由 `appendConfigChangedEvent (ty)` 追加 `config.changed` | 3.3、5.2 |
...
| pi worker 回调 | `session.manage`、`notify.send`、`wake.set` | ViewSessions、Notify、RemindDuoduo 的工具体：`runViewSessionsTool (pg)`、`runNotifyTool (gg)`、`runRemindDuoduoTool (mg)` | 4.1、6.1 |
...
| 有真名的处理函数 | `C.result = await A0e(u, l, d, k)`（`createDaemon`）；`C.result = await xyt(d, F, k)`（`createDaemon`）；`C.result = await Ayt(u, d, k)`（`createDaemon`）；`C.result = await lyt(u)`（`createDaemon`） | confirmed |
| 会话管理类方法的处理函数及其调用的具名函数 | `C.result = await h$(d, c, k)`（`createDaemon`）；`C.result = await byt(u, e.sessionManager, d, k)`（`createDaemon`）；`C.result = await wyt(u, d, k)`（`createDaemon`）；`C.result = await Tyt(u, d, k)`（`createDaemon`）；`let u = await Ybe(e, i)`（`archiveSessionIfQuiescent`）；`i = await ole(e, r, n.display_name)`（`setSessionAliasAndReindex`）；`r === "get" ? void 0 : await ty(e, {`（`applySessionConfigVerb`） | confirmed |
| pi worker 回调方法交给对应的工具体 | `let k = await cg(S.params, {`（`createDaemon`）；`let k = await gg(S.params, {`（`createDaemon`）；`let k = await mg(S.params, {`（`createDaemon`）；`let k = await pg(S.params, {`（`createDaemon`） | confirmed |
