...
| 适配器选择 `$e(w)` | 非 claude→actor 单一 `w.adapter`，未构造则返回抛错替身、失败关闭；claude 非 channel/无 streaming→一次性 `s`；否则长驻 streamingAdapter | `$e`（`83790`） | confirmed |
...
| 抢占三态 accept/tool_use/tool_result + hard/soft；`G()` 置标志、`L(y)` 扳机 abort | `G(y,T,j)` 三分支；`"defer_accept"`/`"defer_tool_use"`/`"defer_tool_result"`；`L(y)` 消费 `activeToolCalls.set/delete` | daemon:71818-71645 / 72691 | confirmed |
...
| partition frontmatter runtime 校验改走共享枚举成员判定 `Aa`（v0.8.0 起四值），非法回退全局默认 | `Aa(a) ? u = a : a !== void 0 && Ee(...)`（`66229`），在分区 frontmatter 解析器 `Zut`（`66217`）内；`Aa` 即 `F0`（`31611`），查的是四值数组 `L0`（`31664`） | `Aa`（`31729`） | confirmed |
...
- **state.json 运行时相关字段**：`sdk_session_id`、`pending_fork_to`（codex 会话下一次 drain 从哪个 SDK session fork；非 codex 或 stateless 时在 drain 起点清掉，`"pending_fork_to"`（`70565`））、`model`、`model_runtime`（写入 model override 时的 runtime）、`pending_model_fork`（drain 起点读 `snapshotModelRuntime: U?.model_runtime`（`70567`）与 `U?.pending_model_fork`（`70570`），runtime 翻转时清掉 override）。
...
- **lint 全程只读、每类每 tick 至多一条、契约门控**：受 `H6`（`68546`）读的 `ALADUO_EXP_MEMORY_CHECK`(check) 与 `ALADUO_EXP_MEMORY_FORGET&&check`(forget，依赖前者) 开关。子步由 `runMemoryCheckTick (Lct)`（`68576-68690`）按名字逐个调用，外层 `aa`（`68700`）在内层 `V6`（`68692`）的 try/catch 之外再统计本步读不到的路径数（有则整拍抑制 inbox sweep）。顺序是：`aa("orphan-states"`（无条件，只算孤儿节点状态、不投递）；check 门内十个产出候选的检查——`aa("board-lint"`（`68618`）、`entity-lint`、`node-lint`、`gap-lint`、`fold-lint`、`broadcast-budget`、`broadcast-lint`、`broadcast-flatten`、`activation-lint`、`aa("orphan-newborn-island"`（`68644`）；十步全部成功时再跑 `V6("inbox-sync"`（`68649`）；forget 门内最后跑 `V6("orphan-forget"`（`68670`））。投递经 `rO`（`67293-67344`；check tick 调用点 `rO`（`68603`））→ posted/withheld（withheld 三因：契约门 `iO` 拒、`already-pending` 存在、写入撞 `EEXIST`）。`routeContractDecision (U6)`（`68509`）按内存节点路径选分区：`topics/` 下 slug 以 `lesson-`/`groove-` 开头的去 `"pattern-tracker"`，其余一律去 `"intuition-weaver"`；它只在 orphan-newborn 路由与 forget 前的警告门 `jve(s, U6(v))`（`68678`）两处被调用。活体 `daemon status`：`memory_check: check=off forget=off (posting governed by partition contracts below)` + 4 条 contract。（confirmed）
...
| memory lint 只读、每类≤1条、受 check/forget 门；路由 `U6` | `orphan-states`（无条件）→ check 门内 board/entity/node/gap/fold/broadcast-budget/broadcast-lint/broadcast-flatten/activation/orphan-newborn-island 十步 → `inbox-sync` → forget 门内 `orphan-forget`；`U6` 路由 pattern-tracker / intuition-weaver | `runMemoryCheckTick (Lct)`（`68576-68690`）、`routeContractDecision (U6)`（`68509`） | confirmed |
