...
外部输入 (channel.message)                                    [→Part III §5 Gateway 入站边界]
   ...
   │      target 由入口 Gle 经 jXe 决定，Kle 内由 qle 读出
   │      gateway → 同步处理不入队
...
**图中各步的代码位置（v0.8.2）**：① `createSpineEvent (rn)`（`32001`），调用点 `createSpineEvent (rn)`（`87216`）；② `computeDedupKey (wse)`（`86907`），调用点 `o = wse(r)`（`87254`）；③④⑤ 同一行 `await sn(e, r), await Nu(e, "gateway"`（`87291`）：`atomicAppendEvent (on)`（`32043`）、`advanceConsumerWatermark (Nu)`（`32858`），status 写入器 `Du`（`32919`）经 `gR`（`32877`）落到 `registryDir/status.json`；⑥ `appendBeforeExecuteGateway (Kle)`（`87214-87346`），target 由 `Gle`（`87161`）内 `s = jXe(t, i, r)`（`87165`）决定，`Kle` 内 `d = qle(r)`（`87303`）读出，meta 分支 `"meta:subconscious"`（`87314`），session 分支 `@evt(${r.id})`（`87325`）；⑦ `n.bus.emit("spine.event", r)`（`87336`）；⑧ `readEventById (Md)`（`32052`）；⑨ `renderPromptLayers (Nhe)`（`55093`）、`buildSystemPromptForChannelConfig (Jh)`（`55120`）、`buildTransientUserBlocks (eke)`（`71668`）；⑩ `drainSessionMailbox (KSe)`（`70371`）、`batchDrainItems (EH)`（`71807`）、`createAgentSdkAdapter (Ef)`（`55213`）、`createCodexAppServerAdapter (yw)`（`61974`）、`createSessionManager (Agt)`（`83651`）；⑪ `last_event_id`（`70960`/`71298`），`sdk_session_id` 在 `Wt.sdk_session_id = nn.sessionId`）与 `vt.sdk_session_id = Nr.sessionId`）；⑫ `transcludeBroadcastBoard (WEe)`（`82197`）。
...
**广播板包装：OVERRIDE 前缀 + dossier 纪律**（confirmed）。第 4 层的 memoryBoard 整段被常量包装（`khe`/`hrt`/`grt`）定义，使用于 `grt.test(d)`（`55106-55112`））：
...
**广播板来源：`@include` transclusion**（confirmed；v0.8.3 更新：递归体从 `transcludeBroadcastBoard` 自身拆成独立内部函数）。`transcludeBroadcastBoard (WEe)`（`82197-82203`）现在只是一层薄封装：以空 `Set` 播种后调用内部递归函数 `JEe`（`82212`，无导出名）解析 `memory/CLAUDE.md`，再用 `hgt`（`82205`）渲染结果。`JEe` 用 `@path` 前缀语法（正则 `UEe=/(?:^|\s)@((?:[^\s\\]|\\ )+)/g`（`82342`））提取 include，按深度上限 `fgt=5`（`82342`；`if(n>=fgt)return[]`（`82213`））递归内联，扩展名白名单 `pgt`（`82342`，仍为 106 个扩展名）。每个被 transclude 的文件头是 `Contents of ${path}${mgt}:`（`mgt=" (project instructions, checked into the codebase)"`（`82342`），渲染器 `hgt`（`82205`）），非裸冒号。

- **去环细节**（confirmed）：visited 集主检的是 `t.has(o)`（`82216`），其中 `o=qEe(i)` 是 resolve+win32 小写后的路径（定义 `qEe`（`82254`）），**不是 realpath**；realpath 由 `ygt`(`82246`)另行求得后以 `t.add(o), t.add(qEe(a))`（`82220`）额外加入 visited 兜住软链别名。即"resolve 主检 + realpath 补检"。

**活体冷启动印证**（confirmed）。本机 `~/aladuo/memory/CLAUDE.md` 为 0 字节 → `He.memoryBoard` 为空 → 不构造 memoryBoard（`memoryBoard: He.memoryBoard ? {path,content} : void 0`））→ `Jh` 不注入第 4 层。这印证机制本身：广播板初始为空，由潜意识逐步写入 durable heuristics 后才在下一次会话被注入——**渐进式冷启动，而非硬编码知识**。
...
**首块 `daemon-restart-hint` 携带重启原因，且只到 channel 会话**（confirmed）。文本由 `renderDaemonRestartHint (Vbe)`（`65850`）拼成：底句 `[system] You're running under a new daemon process (started <ts>).`，若本次启动认领到了重启原因则追加 ` Restart reason, given by the caller: <reason> (requested <ts>).`。原因来自 `getPendingRestartReason (qbe)`（`65813`）——一个模块级全局，由 boot 期一次性认领写入（见 §4 论点一末尾的"跨进程旁路"）。**是否注入由 `qbe`)门控，非 `channel` 类会话直接判 `out-of-scope`**：`job:` / `meta:` / `cadence:` / `subconscious:` / `system:` 会话永远拿不到这个块，因此"重启原因会到达每个被唤醒的会话"这个直觉是错的。另有 `new-session`（无 `lastEventAt`）与 `grandfather`（无水位）两个阶段只写水位不注入。
...
- `qye`（`61929`）/`Bye`（`61947`）全仓仅在 `qye(t ?? {}, h)`（`62052`）/`qye(t ?? {}, …)`）（`yw` 的 run 内）被调用，用的是闭包 `t`（`yw(e,t)` 的第 2 参 = instructions）。
...
**适配器选择**（confirmed）。内层选择器 `$e(w)`）决定路径：runtime 不是 `claude`（即 codex/grok/pi）→ 用 actor 上唯一的 `w.adapter` 字段，三种后端都往这一个字段里填（codex 在 drain 中由 `w.adapter = f({`（`84382`）惰性构造，走上述 `yw`）；该字段为空时返回一个 `run` 直接抛错的替身（`refusing to fall through to Claude`（`83822`）），即失败关闭、不回落到 Claude。runtime 是 `claude` 时，channel 会话走 streaming 包装，其余走裸 `Ef`。
...
**`XEe` 的真正用途：instructions 指纹 / 漂移失效**（confirmed）。`XEe`(`82549`)确实存在——它与 `runInstructionsFingerprintGuard (OA)` **同在一行被调用（`XEe`/`OA`（`84155`）），且 `XEe` 的输出被喂入 `OA` 指纹守卫**（而非"被 `OA` 调用"）。但它**不进任一路的实际提示词**（Claude 用 `jb()`+effective_config，Codex 用 `Jh` 输出）。它的实际用途有二：
1. 产出 `memoryBoard`（`(await WEe(e.memoryBroadcastPath)).rendered.trim()`（`82553`））供 `Jh` 两路复用——经 `memoryBoard: He.memoryBoard ? {…}`）打包成 `{path, content}` 进 drain 配置的 `memoryBoard`；
2. 在调用点 `OA`（`84155`）喂给 `runInstructionsFingerprintGuard (OA)`（`82402`）算 instructions 指纹做漂移检测——指纹 = `computeInstructionsFingerprint (LS)`（`82348`）= `sha256(JSON.stringify([identity??"", kindPrompt??"", instancePrompt??"", memoryBoard??"", mission??""]))`（每元素带 `?? ""` 兜底），**另有一个条件第六元素**：`e.missionAcceptance` 非空时 push 进同一数组（`t.push(e.missionAcceptance)`（`82350`））——即 job 的 acceptance 文本也是漂移信号面的一部分。
...
即 `uot` 的 identity/kindPrompt/instancePrompt 只进指纹、不进提示词——它是驱动 resumed session 失效的独立信号面。此外 `OSe`(`70231`)/ `autoloadAdditionalDirectoryClaudeMd`env 旗标 `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD`（`55254`））门控 memoryBoard 与 additionalDirectories 的 CLAUDE.md 自动加载，是广播板之外第二条"文件即上下文"注入。
...
**三层叠加复用既有的 per-key 折叠器，instance 胜（confirmed）。** `cbe`（`65607`）在装配 effective config 时，把 `runtimeModels` 交给 `obe`）、`runtimeEfforts` 交给 `sbe`），两者传入的都是**定序三元数组** `[{source:"global"}, {source:"kind"}, {source:"instance"}]`（`65673-65681` / `65683-65690`）。二者都只是 `z$`（`65456`）的薄包装——`z$` 顺序遍历各层、对每个 key 逐条覆盖（`n[i] = t(o, r.source)`），故**后面的层赢，且合并是 per-key 的**：在 kind 层设 `codex.model` 不会动到 global 层的 `claude.model`。取值侧是两个一行函数 `Mw`（`65505`）= `config?.runtimeModels?.[runtime]` 与 `jw`（`65509`）= `config?.runtimeEfforts?.[runtime]`，返回 `{model|effort, source}`——`source` 就是"这个值由哪一层决定"，即 `/model` 现在能报出层名的原始出处。

**校验一松一紧，是两种不同的产品判断（confirmed）。** `model_id`（`88009`）只要求 trim 后非空且不含空白（`!/\s/.test(r)`）——model id 是开放宇宙，拒绝未列出的 id 会挡死宿主能触达的任何兼容 endpoint，所以放行，错的 id 留到下一轮真正跑时报错。`effort_level`（`88019`）则严格比对 `qi = ["low","medium","high","xhigh","max"]`（`31538`）五值（v0.8.3 新增 `max`），不中就在命令处直接拒绝并把五个值列进错误信息——词表是封闭的，写错必是笔误，而一个静默生效的错误档位会在每一轮被拒却没有任何东西指认是哪行配置导致的。Claude 模型不支持 `max` 时按 `high` 执行，`/effort max` 的回复会说明这一降级（confirmed，changelog 原话）。

**⚠️ model 与 effort 的优先级并不同构——上游自己的文档写反了（confirmed，逐行核对）。** 两个解析器就差一行：`OSe`）取 `e.jobModel ?? e.sessionModel`，`ASe`）取 `e.sessionEffort ?? e.jobEffort`。**顺序是相反的**：模型上 job frontmatter 压过会话自己的 `/model`，力度上会话自己的 `/effort` 压过 job frontmatter。优先级确实在解析器内部裁决、而非调用方预先解析好——同一处调用点（`Bn = OSe({…}), cr = ASe({…})`））把两组入参都按未解析的原样传入：`jobModel: n.jobContext?.model` 配 `sessionModel: U.model`，`jobEffort: n.jobContext?.effort` 配 `sessionEffort: U.effort`。因为差异只在 `jobContext` 存在时显形，普通通道会话观察不到它，**只有 job 会话会撞上**。两者都在取不到显式值时才落到配置层（`Mw`/`jw`），并把 `configLayer` 置为该层名；显式值命中时 `configLayer` 为 `undefined`——这正是"`/model` 只在配置层决定时才报层名、存了 `/model` 就直接打印"的实现。而上游 skill 文档 `skills/duoduo-runtime-admin/references/model-defaults.md` 宣称 effort 与 model "the same shape"，并把"会话自己的 `/model`"列为最高优先级——**对 effort 成立，对 model 不成立**。以运行时为准。

**"该跑什么"与"实际跑了什么"是两条独立记录（confirmed）。** 解析结果经 `modelOrigin`/`effortOrigin` 落进 drain 记录（批处理路径 `modelOrigin: Bn.configLayer`）/`effortOrigin: cr.configLayer`），live streaming 路径 `modelOrigin: it.configLayer`）/`effortOrigin: bn.configLayer`），均在 `drainSessionMailbox (KSe)`(`70371`) 内）；而真正服务了这一轮的模型由 `MSe`（`70276`）从 usage 里抽出、写成 `last_served_model`（`70965`、`71303`），再经 `createSessionManager (Agt)`(`83651`) 投影成 `lastServedModel`（`85238`）对外可见。**两者可以合法地不一致**——Codex 上一条已开始的会话会保持它启动时的模型直到 fork，所以新设的默认值要到下次 fork 才显形。这条"记录实际服务的模型、而非被请求的模型"的设计，正是 CHANGELOG 所说"a gateway quietly substituting a model is visible instead of invisible"的落地。
...
- **v0.8.2 工具面三处改名与缩权（confirmed）**：`Wake` → `RemindDuoduo`（`Ew`/`y_e`）），`ManageSession` → `ViewSessions`（`kw`/`g_e`））；**`ManageJob` 的 action 枚举收缩为 `create|list|read`）**，`archive`/`interrupt`/`reschedule` 三个动作被移出工具面、下放到 shell（详见 §8 CLI 侧 `duoduo job`）。**v0.8.3 起 `action` 由可省（缺省 `list`）改为必填**：`r == null` 直接 `throw new Error("action is required: create | list | read")`（`cg`（`63807`）），JSON schema 里的 `default("list")` 与对应的 "Defaults to 'list'" 说明一并删除——此前 `action` 在传输途中丢失（部分 Anthropic 兼容网关会重序列化工具入参）时会被当作 `list` 静默放行，看起来像成功；现在任何运行时下都直接报错，不再有这种"看似成功实则未执行创建"的误判（confirmed，changelog #79）。缩权不是静默的：daemon 内置了两段专门的拒绝文案，`yat`) 解释"创建作业属于前台会话或 keepalive 作业，你是执行者不是调度者，要再跑一次请用 RemindDuoduo"，`vat`) 直接说 `ManageJob has no 'reschedule' action.` 并给出两条替代路径。改名对 Codex 会话不是立刻生效——工具名随会话进程的工具表下发，需 `/reset` 后才看到新名（未证实推测：该结论出自上游 changelog，bundle 内未见对应分支）。
...
- **drain 循环本体 = `U`，即 `createSessionManager (Agt)`（`84000`）内的局部函数，再抽循环 `for (; w.status !== "ended" && J;)`（`84125`）**。`drainPromise` 被赋值为 `U(K)`（或先跑 preStart 的 `.then(() => U(K))`）：`K.drainPromise = U(K)`（`84026`/`84027`）；状态字面量里的 `drainPromise: null`）只是字段声明，不是"进入循环"。`KSe` 在循环体内每次迭代调用一次（调用点 `KSe`（`84520`））。
...
- **`batchDrainItems (EH)`(`71807-71843`) 只返回单一 batch**：顺序累积 items 到数组 `i`，遇 `i.length >= u()`（`71819`）（`u` 对 notify 批返回 `Number.POSITIVE_INFINITY`，否则返回 `n.fallbackBatchSize`（`71817`））/ notify-homogeneity 变化 `USe(c) !== a`（`71826`）/ `Math.abs(f - o) > n.mergeWindowMs`（`71836`）三者之一就 `break`，剩余事件留给下一次 `U` 迭代。**"不同目标"不是 `EH` 的 break 条件**，它由下一条讲的可合并谓词裁决。随后 `KSe` 把该单一 batch 解析成事件列表 `yt`，再用可合并门 `sft(yt, t)`）决定走向：合并成立 → **`SH`（`70777`）一次 + `HSe`（`70849`）一次**发一次 query；否则 `for (let ae of yt)` 逐事件遍历 `yt`）各自 `HSe`（`71154`）——"每个可合并批发一次 query"成立，但"一次 `EH` = 若干 batch"不成立。
...
- **适配器选择 `$e(w)`，即 `createSessionManager (Agt)`（`83790`）内的局部函数**：runtime 不是 `claude` → 取 actor 的单一 `w.adapter` 字段（codex/grok/pi 共用），未构造时返回一个 `run` 抛 `${w.runtime} runtime selected but its adapter was not built` 的替身，失败关闭而不回落 Claude；runtime 是 `claude` 且 `origin!=="channel"` 或无 `createStreamingQuery` → 一次性适配器 `s`；否则惰性长驻 `streamingAdapter`（经 `x(w, P)` 创建）。
...
**(e) hold-stdin——后台 subagent 续跑。** drain 传 `holdInputOpenForBackgroundAgents = runtime==="claude" && origin!=="channel"`（`w.runtime === "claude" && w.origin !== "channel"`（`84531`），`KSe`（`70705`）透传）。非流式 `run` 中 `x = t.holdInputOpenForBackgroundAgents === !0`（`55350`），prompt 换成 `Ef`（`55377`）内的生成器 `J()`（`for await ... yield U; await $`）：yield 完 prompt 后 `await $` 让 stdin 不关，后台 subagent 完成回调（in-process MCP）仍可送达。`$` 的 resolve（`C`）走 `A || D && S.size === 0 && (A = !0, L(), C())`（`55364`）——**需同时满足 `D`（已收到 result，`U.type === "result" && (D = !0, B())`（`55486`）置位）与 `S.size===0`**（`S` 增删来自 SDK system 事件 `task_started`/`task_notification`），非仅"后台 task 跑完"。idle 看门狗 `j = fB(process.env.ALADUO_HOLD_INPUT_IDLE_TIMEOUT_MS, 6e5)`），默认 10min，触发打印 `hold-input idle watchdog fired`（`55371`）；另有 abort-close 看门狗 `I = pB(process.env.ALADUO_ABORT_CLOSE_TIMEOUT_MS, 1e4)`（`55347`）。

> **Skip 语义在 v0.6.2 从"中断"降为"hook 裁决"**（confirmed）。v0.6.1 里 hook 只记标志，真正结束 turn 的是随后在 `tool_result` 边界调用的 SDK `query.interrupt()`（流式带调试标签 `"anchor-turn skip"`，非流式打 `[claude-sdk] Skip called — interrupting turn (non-streaming)`）——这次中断顺手拆掉了本 turn 内启动的全部后台 task。v0.6.2 让两条路径的 PreToolUse hook 直接**返回** `{continue:!1, stopReason:"The agent intentionally ended this turn silently by calling Skip."}`（流式 `o0e`），非流式 `Ef`（`55289-55294`）），interrupt 调用与 `interruptRequested` 字段整条删除；`query.interrupt()` 只剩会话级用途（真实用户打断 / 会话回收，`e.query?.interrupt()`（`82837`））。**后台 worker 能活下来不是因为新增了保护，而是因为杀死它们的那个动作没有了。**
>
...
| 合并谓词（同目标、无斜杠命令、notify-homogeneous）是合并的真正边界 | `cft`（`length < 2` 拒；全 channel.message 走 `dft`，全 notify 走 `nke`）/`dft`（`r.startsWith("/")` 拒）/`nke`（`pft` 目标键 `n.size === 1`）/`rke`（notify 判定）/`pft`（`primaryTargetSessionKey\|fanoutTargets\|…` 键）；门 `sft(yt, t)` | `cft`（`71862`）、`dft`（`71866`）、`nke`（`71874`）、`rke`（`71880`）、`pft`（`71909`）；`if (sft(yt, t))`） | confirmed |
...
| 适配器选择 `$e(w)` | `w.runtime !== "claude"` → `w.adapter`（未构造则返回 `run` 即抛错的替身 "refusing to fall through to Claude"，失败关闭）；`w.origin !== "channel" \|\| !s.createStreamingQuery` → 一次性 `s`；否则惰性长驻 `w.streamingAdapter`（经 `x(w, P)` 建） | `$e`）；`createSessionManager (Agt)`（`83790-83821`） | confirmed |
...
| Skip 联动跳过 SDK 结果并抑制 outbox | hook 内 `le.skipCalled = !0`；drain 侧 `nn.skipped && (Ro.skipped = !0, B = !0)` / `Nr.skipped && (Yf.skipped = !0, B = !0)`；"[runner] Skip called — suppressing outbox" | `le.skipCalled = !0`（`83101`）；`nn.skipped && (Ro.skipped = !0, B = !0)`（`70901`）；`Nr.skipped && (Yf.skipped = !0, B = !0)`）；`"[runner] Skip called — suppressing outbox"`（`71228`） | confirmed |
...
| 执行事件包装器 `He` 统计工具计数 + compact_boundary，喂 drain record/spine | `_e.type === "tool_use" ? l += 1 : … _e.isError && (c += 1)`；`_e.subtype === "compact_boundary"` 捕获 | `He`）；`_e.type === "tool_use" ? l += 1`（`70765`）；`_e.subtype === "compact_boundary"`（`70756`） | confirmed |
...
- **内建 hooks**：三个 hook 同属一个字面量块，整块在 `o0e`)——即长驻流式 query 的 options 里，不是散落各处。PreToolUse matcher `"*"` 记 `transcript_path`（`o0e`)，同样用 `agent_id === void 0` 判定是否主 agent）；matcher 为 `cc`(`54716`)（即 `mcp__aladuo__Skip`）的 Skip hook 带 `agent_id` 子代理护栏并**返回** `{continue:!1, stopReason:…}`（`o0e`)）。PostToolUse matcher `"*"` 做 steer 注入（`o0e`)），v0.6.2 在其顶部新增两个提前返回：`currentTurn?.skipCalled === !0` 或 `cliTurnTentative?.skipObserved === !0` 时直接 `return {}`（`o0e`)），即**已 Skip 的 turn 不再消费 `pendingSteer`**——v0.6.1 会把 park 中的插话 markDone 并注入一个即将被丢弃的 turn，插话就此消失。（v0.5.8 里"PreToolUse matcher `Bash` 拦截 `run_in_background`"的 hook 自 v0.6.1 已不存在。）
...
- **一 key 一 actor**：注册表 `let B = new Map`），`N`）负责生成 actor，`actorRunId: V=++ee`）单调自增，`B.set(w, K)`）。活体 `system.status` 只有单一 actor（status=`idle`），印证"至多一个"。**confirmed**。
- **kind 的第二真值来源**：actor 新建函数 `N`在 `createSessionManager (Agt)` 内）调 `OR(t, {...})`（声明 `OR`@`35474`）把 `display_name/kind` upsert 进 meta.md（调用点 `OR`@`84010`），此处 `kind` 从 **origin** 二次派生（`origin==="job"?"job":origin==="system"?"system":startsWith("meta:")?"meta":"channel"`（`84013`））——与 `qr` 的前缀派生**并存**，是 kind 的另一条真值来源。**confirmed**。
...
- **进程级 runtime 写锁**——保证同一 runtime_dir 只有一个 daemon 写者。锁文件 `run/locks/daemon-writer.json`（路径由 `d6`@`88831`），记 `{runtime_dir, pid, boot_id, started_at, last_heartbeat_at}`。`start()` 内 `$=await f6(u); if(!$.acquired) throw new Error(…Runtime lock already held by pid=${...}…)`）；心跳 `setInterval(()=>eve(u)…,C)`、`C=T0e("ALADUO_RUNTIME_LOCK_HEARTBEAT_MS",3e4,1e3)`）。夺锁前 `Vut`@`88866` 判 stale，逐字符为：
  ```
...
- **重启 actor 的 origin 从何而来（"重抢槽"闭环的落池决策）**：出队函数 `L`（`83697`）与唤醒函数 `se`）回落新建时都先算 `ZW(key)`，推断出 origin 就走 `N(key, origin)`，否则 `N(key)`——`GW`（`80247`，两处调用点：`L` 内 `GW`、`se` 内 `GW`）按 `job:`/`meta:`/`cadence:`/`system:` 前缀推断被出队/唤醒 actor 应落 channel 还是 job 池，是"重抢槽"闭环里决定池归属的关键；真正的选池谓词是 `j`）里的 `bEe(key,origin)==="job"?jobPool:channelPool`（定义 `vEe`@`80259`）。**confirmed（机制），origin 推断细节为静态阅读**。
...
- **唤醒与抢占 `Ne`→`Ue`/`N`→`G`（现为 `se`→`zS`）**：`se`）默认 `z=P?.preempt??"allow"`（`83867`）；归档会话 wake 被抑制（`or(w)`→`wake suppressed, session is being archived`（`83832-83862`））；idle actor 直接 `V.wakeResolver()`（`83876`）。`zS`（`82849`）在 `tool_use/tool_result/accept` 边界延迟中断，`t==="soft"` 分支存在，返回 `defer_*/immediate/noop`；`force→zS(V,"immediate",F,"preempt")`（`83896-83897`）、`allow→zS(V,"soft",F,"preempt")`（`83912-83913`）。**confirmed**。
...
- **收尾再校验 `He`→`A` + 一次性重驱**：`sessionInboxFreshNameVerdict`（`80613`）actor end 后重扫 inbox，返回 `"fresh"/"conservative"/"none"`（`80293-80297`）；`fresh`→重新 wake（`preempt:"never"`））；`conservative`(瞬时读失败) 受 `consecutiveConservativeRedrive` 约束——该字段**确为布尔**：初始 `consecutiveConservativeRedrive ?? !1`（`84004`），已 true 则 `conservative re-drive suppressed (cap spent)`（`84847`），首次则 `=!0`+`re-entering wake path once`（`84850`）——即只重驱一次防自旋（`consecutiveConservativeRedrive`（`84840-84855`））。**confirmed**。
...
|---|---|---|---|
| 进程级 runtime 写锁 + pid/boot_id/心跳/TTL 抢占 | `daemon-writer.json`；`Runtime lock already held by pid=${...}`；`knt`: `Number.isNaN(r) \|\| t.getTime()-r>n \|\| (e.boot_id && e.boot_id!==Zhe()) \|\| !wnt(e.pid)`；`wnt`=`process.kill(e,0)`；TTL `ttlMs??12e4` | daemon `c6`/`Lut`/`Fut`/`Qbe`/`zut`/`f6` TTL；抢锁 `p6`）；心跳 `ALADUO_RUNTIME_LOCK_HEARTBEAT_MS`（`91488-91491`） | confirmed |
...
| 一 session_key 一 actor + 单调 actorRunId + active/idle/ended | `let B = new Map`）；`N()` 生成、`actorRunId: V=++ee`；`status:"active"→"idle"→"ended"` | daemon `actorRunId`（`83931-83937`）/`w.status = "idle"`（`84728`）/`w.status = "ended"`（`84837`） | confirmed |
| 双有界池：channel=10 / job=6，超限入 wakeQueue | `C`/`$` `{name,activeCount,maxConcurrent,wakeQueue}`）；`D=…??10`、`A=…??6`；`K.wakeQueue.push(w)` | daemon `maxConcurrentJob`）；`wakeQueue.push`（`83941`）；RPC `system.config`→`max_concurrent_channel:10, max_concurrent_job:6`（`89157-89158`） | confirmed |
| idle 释放池槽 + `hJ`(idle_ms) 等待，前台附着钉住 actor | `released pool slot (idle)`；`mJ(e,t){ wakeResolver=n(!0) vs setTimeout(n(!1),t) }`；`idle timeout with attachments, reclaiming runtime processes`（其后 `continue`） | daemon `released pool slot`（`84741`）/`hJ`（`82878-82890`）/`idle timeout with attachments`（`84765`）；重抢槽 `re-acquired pool slot`）；`idle_ms:3600000` RPC | confirmed |
...
- **选择链 A——`createSessionManager` 内部通用解析器 `i(a,l)`（`QEe`）内）**：`if (l?.runtime === "grok") return "grok"`（`82661`，**显式声明 grok 直接返回，不做可用性门控**）→ `if (l?.runtime === "codex") return "codex"`（`82662`，**codex 同样直返不门控**）→ `if (l?.runtime === "pi") return "pi"`（`82663`，**v0.8.0 新增，pi 与 grok 同组**）→ 有 `source_channel_id` 则回溯 channel descriptor/kind 配置取 frontmatter `d`，最后 `d ??= ao(), d === "grok" ? "grok" : d === "pi" ? "pi" : d === "codex" && (await n()).ok ? "codex" : "claude"`）——**frontmatter 派生的 grok、pi 都不经过 `(await n()).ok` 门控，只有 codex 才检查**（confirmed，逐行核对）。
...
- **pi = 内嵌 worker 子进程，是三个非 Claude 后端里唯一"随 duoduo 一起装好"的**：worker 入口由 `eS`（`72731`）解析：`IH(e, "pi-worker.js")`）存在则以 `process.execPath` 运行，否则回退 `args: [IH(e, "pi", "worker.ts")]`），三处 actor 创建点都传 `workerCommand: eS()`）——spawn 的是 duoduo 自带的 pi coding-agent SDK 里的 worker 脚本，不是像 codex/grok 那样 shell 出用户 PATH 上的外部 CLI，所以**没有对应的 `checkPiAvailability`**（全文件搜索确认不存在）。它没有 claude 式的进程内 SDK 调用，也没有 codex/grok 式的探测-缓存三件套，是第四种、更简单的执行形态：直接假定可用，把"能不能用"完全下放给消息发送时的 provider/模型解析（confirmed，结构化 grep 验证不存在 pi 专属可用性探针）。
- **v0.8.3 起四个运行时同一套失败模式：先绑定，再探测，不可用就拒绝，不再有任何静默换后端**（confirmed，取代 v0.8.2 的"codex 静默降级、grok 绝不降级、pi 无从谈论降级"三分）。`createSessionManager (Agt)` 内未导出的 spawn 逻辑对 job/channel 两种 origin 是同一套模式：job 分支先解出目标运行时 `pi`（frontmatter 显式指定或走默认解析），`w.runtime = pi`（`84098`）无条件赋值在前，随后一行 `await v(pi)` 才探测可用性；不可用时只记 `Ut = Bn.reason` 并打印通用警告 `job requested ${pi} but it is unavailable`（`84100`），不再把 `w.runtime` 改回 `claude`；channel 分支同构：`w.runtime = Di`（`84115`）在前，随后一行 `await v(Di)` 探测可用性，警告 `channel requested ${Di} but it is unavailable`（`84117`）。可用性从不被"记住"：`v(pi)`/`v(Di)` 每次进这段都重新探测，装上 Codex CLI 后下一条消息立即生效，无需重启。真正的拒绝发生在 `drainSessionMailbox (KSe)` 内两道顺序门：先 `stage: "runtime_unavailable"`（`70714`，`It`/`pi` 非空即拒），再 `stage: "runtime_mismatch"`（`70731`，会话已绑定的 `sdk_session_runtime` 与本次请求的 runtime 不一致即拒，提示文案含"Session histories cannot move between runtimes, so choose one:"）；drain 循环见到 `refusedStage` 命中这两者之一就打印 `runtime refusal, ending actor`（`84704`）并结束 actor，不会带着错误的 runtime 继续跑。即"探测什么时候查不重要，谁都不能带着错的运行时往下走"——这也是 changelog 里"切换运行时先 `/clear`"这条新规则的机制来源：同一份 SDK 会话历史一旦绑定了某个运行时，换运行时前必须先清空历史，否则直接拒绝而非默默把历史喂给不兼容的另一个运行时。
...
- **`/model`——Claude 试图即时、Codex 只能延迟 fork**：`setSessionModel` 内 `if (V === "codex")`（`V = await I(w, F)`）→ 写 `rt(...model_runtime: P!==null?"codex":null, pending_model_fork:!0)` 并返回 `applied:"stored"`）；claude 路径 `Re="stored"; setModel 成功→Re="live"`，写 `model_runtime:"claude", pending_model_fork:null, applied:Re`）。codex 回执 `Codex session — a switch takes effect from the next message.`（`87651`/`87723`）。runtime flip 时经 `!(i ? i!==o : o==="codex" || o==="pi")` 守卫清空 `model/model_runtime/pending_model_fork`（`72299-72302`）（confirmed）。
...
- **`/compact`——门控只锁 claude，实际抵达回执的是 codex**：外层守卫 `ae.event.routing_hint?.intent === "history-control"`（`71000`）；内层 `if (ou === "/compact" && (n.runtime === "claude" || n.runtime === void 0))`（`71017`）→ `Eo(t)==="channel" ? Ze=!0 :（发 "only available in interactive sessions" + continue）`）。`Eo`）把 `job:→"job"`、`meta:→"meta"`、`system:|cadence:→"system"`、含 `:` → `"channel"`，否则 unknown。**codex 根本不进这个拦截块**（守卫限 claude/void），落到命令层 `xft`(`72119`)→ `r.compact()` → `📦 History compacted (runtime: ${o.runtime})`（`72134`）。所以：claude channel 走 `Ze=true` 透传 SDK 原生（不产该串），claude 非 channel 被拦，唯 codex compact 抵达 `xft`（`72134`）的回执（confirmed，codex 路由链已补全）。
...
| pi 无独立可用性探针 | claude/codex/grok 各有 `probeClaudeAvailability`/`checkCodexAvailability`/`checkGrokAvailability` + 结果缓存 + 测试注入，pi 没有对应物（`PiAvailab` 忽略大小写全文件 0 命中）；worker 入口由 `eS` 解析：`pi-worker.js` 存在则以 `process.execPath` 运行，否则回退 `tsx` + `IH(e, "pi", "worker.ts")`，内嵌 spawn | `eS`（`72731`）；`IH(e, "pi", "worker.ts")`）；`workerCommand: eS()`）+ 全文件 grep | confirmed |
...
| claude=进程内 SDK（顶层静态 import，无子进程） | `import { query as She } from "@anthropic-ai/claude-agent-sdk"` | `She`） | confirmed |
...
| **不对称发现：codex 不可用静默降级 claude，grok 不可用绝不降级、留到 drain 硬报错，pi 从不检测、无从降级** | codex 分支 `$r.ok ? w.runtime = "codex" : (w.runtime = "claude", 警告)`；grok 分支先 `w.runtime = "grok"`，再 `$r.ok \|\| (br = $r.reason, 警告)`、不改 runtime；pi 分支仅 `w.runtime = "pi"`，无 await/无判断/无警告 | job origin：`createSessionManager (Agt)`（`84071`）、`w.runtime = "grok"`）、`br = $r.reason`）、`He === "pi"`（`84086`）；channel origin 对称：`_e.ok ? w.runtime = "codex"`（`84098`）、`ln === "pi"`） | confirmed |
| pi 专属的晚发作失败是"没有模型指针"，不是"运行时不可用" | `This pi session has no model yet. Request was not executed. ... Nothing to install: the pi runtime ships inside duoduo.`（与 grok/claude 同在错误文案函数 `Rft` 的按 runtime 分支里） | `This pi session has no model yet`（`72361`）；`Rft`） | confirmed |
| grok 不可用的硬报错文案在 drain 阶段抛出，定制到具体 runtime | `Rft(e, t)` 按 `t` 分支：`"Agent runtime 'grok' is unavailable. Request was not executed."` + `` Install the grok CLI, run `grok login`, then send the message again. ``；drain 中 `n.runtimeUnavailableReason` 非空时 `guidance: Rft(Pn, vr)`，`outcome: "runtime_unavailable"` | `Rft`）；`drainSessionMailbox (KSe)`（`70711-70743`） | confirmed |
...
| 选择链 A：显式 codex/grok/pi 均直返(不门控)，派生 codex 才门控、派生 grok/pi 不门控 | `i(a, u)`：`u?.runtime === "grok"` → `"grok"`；`=== "codex"` → `"codex"`；`=== "pi"` → `"pi"`；派生 `d ??= Co(), d === "grok" ? "grok" : d === "pi" ? "pi" : d === "codex" && (await n()).ok ? "codex" : "claude"` | `QEe`）；`u?.runtime === "grok"`（`82661`）；`u?.runtime === "pi"`（`82663`）；`d ??= Co()`） | confirmed |
...
| /effort 每会话推理力度（v0.6.0，thinking 面新增开关） | `qi = ["low", "medium", "high", "xhigh", "max"]`（`31538`）；网关 `t.name === "/effort"`（`87700`）经 `r.getSessionEffort(n, i)`（`87713`）、`r.setSessionEffort(n, a)`（`87755`）；drain 以 `effort: q.effort`）下达 run-config，codex 侧落为 `turn/start` 的 `effort: I`（`62273`） | `createSessionManager (Agt)`（`85427`）；`drainSessionMailbox (KSe)`（`70705`） | confirmed |
...
| 路由分叉：target 在入口 `Gle` 经 `jXe` 决定，`Kle` 内只读取 | `s = jXe(t, i, r)`（`87165`）；`Kle` 内 `d = qle(r)`（`87303`）后按 `routing_hint.target ∈ {gateway,meta,session}` 分支，meta 写 `"meta:subconscious"`（`87314`） | `Gle`（`87161`）；`jXe`（`87155`） | confirmed |
...
**索引与随机读（confirmed，v0.8.1 复核结论有更正）。** 全仓检索 `by_session` 字面量已零命中（新旧两版 `eventsIndexDir` 下都只挂了一个索引文件），**"两个 append-only 索引"这一旧描述不成立**：只有单个 `by_id.jsonl` `{event_id, partition, byte_offset, byte_len}`（路径 `tb`(`32031`)，写入侧 `Q9e`(`32034`)，字段体见 `atomicAppendEvent (on)`（`32046-32050`）内的 `Q9e` 调用点）懒加载入内存 Map 并随 append 增量更新，支撑随机读 `read(o,0,t.byte_len,t.byte_offset)`（随机读实现 `e5e`(`32060`)，读在 `i.read`(`32067`)，经派发器 `readEventById (Md)`(`32052`) 对外暴露）。首次访问由 `i5e`(`32122`) 整文件流式 load 进 Map（缓存槽 `X9e`)），之后随 append 增量更新——**这正是「随机读 O(1)」的前提**。两级回退（机制 confirmed）：索引命中但按偏移读不出该事件时，`e5e` 在同一分区文件内顺序扫描（`ise`（`32076`）调用点）；索引未命中且调用方给出 `notAfter` 时，`readEventById (Md)` 退到 `scanPartitionsForEventId (t5e)`(`32078`)，按日期倒序逐个分区顺序扫描，没有 `notAfter` 则直接返回 `null`。
...
**消费者 watermark（confirmed）。** `advanceConsumerWatermark (Nu)`(`32858`) 先 `nb`(`32859`/`32095`，前为调用 `nb(e,n)`、后为定义) 经 by_id 反查偏移，再经 `$5e`(`32855`) 写 `R5e`) 拼出的 `run/queue_offsets/<consumer>.json`，字段 `{updated_at, partition, byte_offset, last_event_id}`（活体 `jobs.json` 逐字段吻合）。全部 `hl()` 调用点仅 6 处，对应三个消费者：
...
**所以呢**：v0.7.0 之前，整个 daemon 对外只有一个可确定的物理边界——一个绑在 `127.0.0.1` 的端口、一条巨型 if/else 链，安全边界完全押在「本机单用户」假设上。v0.7.0 把这个边界拆成三层监听器（详见 ARCHITECTURE §10.2 的 TCP 只读 / unix socket 全权 / 可选 token 远程），但**没有把 if/else 分发链拆成三份**——三个监听器全部由同一个路由构造闭包 `R(app,{hostGuard,readOnly,bearerToken})`（`daemon.pretty.js:91180-91374`）挂载路由，只是分别传入不同的 `{hostGuard,readOnly,bearerToken}` 参数（三次调用见 `hostGuard`））。这让"审计一条 if/else 链"的心智模型继续成立，只是现在要多问一句"这条链挂在哪个监听器、`readOnly` 是否为真"。

- **`readOnly` 参数是新的核心分支点。** 只读模式下 `/ws` 直接 426（引导去 socket）、`/rpc` 只放行 `system.status/usage.get/job.list/spine.tail/system.runtime.info/system.config` 六个方法（白名单 `ryt`（`89065`）；其余 `-32601`，HTTP 200，非连接层拒绝）；`/healthz`/`/dashboard`/`/readyz` 三个端点不受 `readOnly` 影响，任何监听器都注册（`S.get("/healthz"|"/dashboard"|"/readyz")`））。三次调用分别是：`R(t,{hostGuard:!0,readOnly:!0})`（`91424-91426`）挂常驻 loopback TCP、`R(n,{hostGuard:!1,readOnly:!1})`（`91427-91429`）挂 unix socket、可选 `R(r,{hostGuard:!1,readOnly:!1,bearerToken:D.token})`（`91477-91480`）挂 opt-in 远程监听器。
  - **`readyz` 语义要点**：探针实为「能否 append 到 events 日志文件」（打开当日 events jsonl 追加句柄再关闭），未就绪回 `503 not_ready`——它探的是 spine 写入能力，而非泛化的「服务活着」（confirmed，具体探针函数行号需结合 §5 论点 3 的 `Kle` 一带核对，未逐字重新定位，标注**未证实推测**具体行号，机制本身 confirmed）。
...
- **真正的 target 决策在入口 wrapper `Gle`，不在 `Kle` 内。** handler 收到 channel.message 后先由 `Gle`(`87161`) 做**斜杠命令解析**再调 `Kle`——`Gle` 里解析出命令/意图后喂 `jXe`(`87155`)裁 target：带参内容 → `session`，`status/config/debug` 类 intent 或纯命令 → `gateway`（内联短路、不进模型），`history-control` 类（`/compact /clear /undo`）→ `session`（`87159`）。这一层命令预处理是「洞察 #5」的真实机制所在。网关命令清单常量 `Ble`(`87085`)已含 `/effort`（每会话推理力度开关）与 `/model` 等。
- **`qle` 只是读取器，不是决策器。** `qle`(`87075`)仅 `return routing_hint.target ∈ {gateway,meta,session} ? … : "session"`——它读回 `Gle`/`jXe` 已写进 `routing_hint` 的结果。`Kle` 内 `d = qle(r)`（`87303`）是**落点**，据此分三路。
...
| readyz 探针 `Kle` = 能否 append events 日志，否则 503 not_ready | `await Kle(u) ? {status:"ok"} : j.code(503).send({status:"not_ready"})`；`Kle` 以 `"a"` 打开当日 `eventsDir` 文件再关闭 | `S.get("/readyz", async ($, j) => await Kle(u)`）、`status: "not_ready"`（`91236`）；`Kle`） | confirmed |
| 只读 TCP 端口默认 20233、host 恒 127.0.0.1；unix socket 默认 `<runDir>/daemon.sock` mode 0600 | `Number(process.env.ALADUO_PORT ?? process.env.PORT ?? 20233)`；socket 路径 `e.daemonSocketPath ?? t.ALADUO_DAEMON_SOCKET ?? join(u,"daemon.sock")`，父目录须 0700、socket chmod 0600 | socket 路径 `J = e.daemonSocketPath ?? t.ALADUO_DAEMON_SOCKET ?? kt.join(l, "daemon.sock")`（`66125`）；父目录 0700 校验 `"daemon socket directory must be owned by this user and mode 0700"`）；`Ms.chmod(x, 384)`（`91472`）；TCP 监听 `host: "127.0.0.1"`（`91474`）、`Number(process.env.ALADUO_PORT ?? process.env.PORT ?? 20233)`（`91592`） | confirmed（含活体） |
...
| routing target 决策在 `Gle`→`jXe`（斜杠命令/intent），`qle` 仅读取器 | `jXe`：带参命令→session、history-control→session、intent status/config/debug 或纯命令→gateway；`qle`：`t==="gateway"\|\|t==="meta"\|\|t==="session"?t:"session"` | `jXe`（`87155`）、`Gle`（`87161`）、`qle`（`87075`） | confirmed |
...
| channel.ack 双路径提交 + -32602 invalid cursor / -32002 归档 | `code: -32002`（`90878`）归档守卫；`session_key` 的 `:` 前缀作 channel kind，`La(u, ce, B)`（`90886`）直读记录文件；未命中或 session 不符 → `ple(u, k.session_key, B)`（`90887`）查记录、`zV` 提交，查不到回 `"Invalid cursor"`（`90895`）/-32602；命中 → `ea`）查 by_id 索引、缺则 `jR(u, k.session_key)`（`90900`）重建，再 `k_e(u, k.session_key, L, ee)`（`90902`）或 `zV` 提交 | `"channel.ack"`（`90874`） | confirmed |
...
（相关文件：`createDaemon (Lyt)`（`daemon.pretty.js:90469-91450`，fastify/RPC/WS）、`jXe/Gle`（`87155-87185`，入站 routing 分流）、`appendBeforeExecuteGateway (Kle)`（`87214-87346`，WAL 入站）、`"session.wake"`（`88557`/`89803`/`90768`/`90818`，session.wake 触发点）、`outboxDir`（`35911-36143`/`36456`，outbox 落盘与索引）、`l6`（`88639-88808`，订阅扇出/降级）；`@openduo/duoduo/node_modules/@openduo/protocol/src/{rpc,channel,outbox,notifications,channel-binding}.ts`。）
...
- **37min 心跳**：`let B=T0e("ALADUO_CADENCE_INTERVAL_MS",222e4,1e3)`定义 `C0e`（`89082`）），env 可覆盖、带 `1e3` 最小 clamp（低于 clamp 或非整数则告警回退默认值）；活体 `duoduo daemon config` → `interval_ms: 37min (2,220,000ms) (default)`。单个 `setInterval`（`91629`）回调体第一个表达式就是 `h.emit("cadence.tick")`（`91630`），**在同一行 `if(…, G){…skipped…;return}` 重入门之前**；随后 `G=!0`、`o(d).then(...).finally(()=>G=!1)`（`91634-91643`）才跑维护环（`o`=`runCadenceTick (Zgt)`（`86463`））。故潜意识总线不受维护环 `K` 阻塞。（confirmed）
...
- **门一 · 重入 + 停机**：`if(p||m){…"[meta-session] skipping tick"…;return}`）——`p`=processing、`m`=stopRequested，**独立于维护环重入标志 `G`）的第二套门**。（confirmed）
- **门二 · 内存指纹活动门**：`let[S,D,A,C]=await Promise.all([FA(memoryFragmentsDir),FA(memoryEntitiesDir),FA(memoryTopicsDir),Lgt(t)]),$=[…].join(":"),j=jgt($)`）；`if(y!==null&&j===y){…"activity gate: skipping tick (fingerprint unchanged)"…}`（`86283-86284`）整跳。指纹分量在 v0.8.1 是 **4 项**（三个记忆目录 + `Lgt`），v0.7.1 基线为 5 项。（confirmed）
- **门三 · cooldown / backoff（round-robin 选择器 `_`））**：逐项判定收在纯函数 `E`）：已勾选 `if (j.done) continue`（`86260`）；分区缺失或停用 `if (!k || !k.schedule.enabled) return j`（`86262`）（交由 `_` 用 `H$`（`85905`）勾掉推进）；退避中 `_6`）→continue；cooldown 判定：`B=Math.max(0,k.schedule.cooldown_ticks),G=b.get(j.name); if(G===void 0||A-G>=B) return j`）（`b`=分区→上次 tick 序号 Map，`b = new Map`（`85890`））。失败退避 `b6`（`66451-66462`）是**线性、非指数**：`success/invalid_output→null`（`66452`）；timeout `if (t <= 2) return null; let s = Math.min(t * i, Qut)`（`66455-66456`）；error `if (t <= 1) return null; let o = Math.min(t * 2 * i, elt)`（`66459-66460`）；常量 `Qut=72e5(2h)/elt=144e5(4h)/_g=222e4`）。`_6`(`66447`)读 `backoff_until` 判定退避中；`_` 选取后再读一遍全分区 state，用 `_6` 过滤出本拍 `backedOff`（`85920-85923`）。（confirmed）
- **选取 + 空闲补跑**：`L = await Fgt(t, r), B = await _(k, L, v)`）；`if(B?.name&&d>1&&(!r||r.activeCount()<=1)) for(ce=1;ce<d&&await _(k,L,v);ce++)`（`86297-86298`），`d=maxPartitionsPerIdleTick??2`（`85890`）。（confirmed）
...
| 单 `setInterval` 同拍先 emit `cadence.tick` 再跑维护环，emit 在重入门 `G` 之前 | `h.emit("cadence.tick"); if(...,G){...return}; G=!0; o(d)...finally(()=>G=!1)` | `setInterval`（`daemon.pretty.js:91629-91645`） | confirmed |
| cron 扫描 `scanAndSpawnDueJobs (yJ)` 属独立 60s job-scheduler（默认 `Hgt=6e4`），非维护环 | `k=s({...}); k.start()`；`createJobScheduler (Ygt)` 内 `yJ` 到期扫描 | `k.start()`（`daemon.pretty.js:91611-91616`）、`createJobScheduler (Ygt)`（`86655`）、`yJ`（`86669`）、`Hgt`） | confirmed |
...
| broadcast lint 已下沉进 `runMemoryCheckTick`；cadence 队列合并（`mergeCadenceInbox`/`parseCadenceQueue`/`queue.md`）在 v0.8.0 整条移除 | `aa("broadcast-lint",()=>d(lwe(u).selected))`；`cadenceInboxDir`/`cadenceQueuePath`/`queue.md` 在 v0.8.1 bundle 零命中 | `runMemoryCheckTick (qct)`（`daemon.pretty.js:68573-68687`）、`"broadcast-lint"`（`68631`）、`cwe`（`68228`/`68632`）、`"claude-lint.md.pending"`（`68244`）、`yct`） | confirmed |
...
| 门一 重入/停机：`p`(processing)/`m`(stopRequested) 独立于维护环 `G` | `if(p||m){...skipping tick...;return}` | `"[meta-session] skipping tick"`（`86273`） | confirmed |
| 门二 活动门：内存指纹（三个记忆目录 + `Lgt`，共 4 项）不变则整跳 | `let[S,D,A,C]=await Promise.all([FA(...),FA(...),FA(...),Lgt(t)]),$=[S,D,A,C].join(":"),j=jgt($)`;`if(y!==null&&j===y){...activity gate...}` | `j = jgt($)`）、`"[meta-session] activity gate: skipping tick (fingerprint unchanged)"`（`86284`） | confirmed |
...
| `_` 用 `_6` 读 `backoff_until` 过滤本拍 backedOff | `_6` 判退避；`backedOff` 报告 | `_6`（`daemon.pretty.js:66447`/`85920`）、`backedOff`（`85923`） | confirmed |
...
| 分区无状态、超时=max_duration_ms、四分类 success/invalid_output/timeout/error（`zgt` 判 invalid_output） | `persistSession:!1`;`K=Math.max(1,max_duration_ms)`;`Promise.race`;`$ = Dgt(S.name, l0e(text)) ? "invalid_output" : "success"` | `persistSession: !1`（`86083`）、`K = Math.max(1, S.schedule.max_duration_ms)`（`86064`）、`j = await Promise.race([Fe, yt])`（`86120`）、`$ = "timeout"`）、`$ = Dgt(S.name, we)`）；`zgt`（`85712`，定义） | confirmed |
| 成功/失败落 Spine + usage drain record | `agent.result tick_type:"subconscious"`;`agent.error stage:"partition_execution"`;`Qd` drain `cancelled:$==="timeout"` | `tick_type: "subconscious"`（`86202`）、`stage: "partition_execution"`（`86223`）、`appendDrainRecord (Qd)`（`86177`）、`cancelled: $ === "timeout"`） | confirmed |
...
| 出厂四分区 = 用户数据分区，各自 frontmatter 覆盖代码里的 schedule 默认 | 如 `pattern-tracker` 的 `schedule:{enabled:true,cooldown_ticks:7,max_duration_ms:900000}`；代码默认 `m6={enabled:!0,cooldown_ticks:1,max_duration_ms:6e4}`（60s） | `subconscious/*/CLAUDE.md`；`m6`)；`duoduo daemon config` | confirmed |
| 契约门 `iO`：6 拒因 + null 放行，双重角色（逐项 + 整拍短路） | `kind-not-consumed`/`partition-absent`/`self-id-mismatch`/`partition-disabled`/`no-contract`/`parse-fail`；`if(!a&&!r) return o` | `enforceContractGate (iO)`（`daemon.pretty.js:67343-67355`）、`Mve`、`a = Fct(() => Mve(s)); if(!a&&!r) return o`整拍短路） | confirmed |
| memory lint 只读、单类限量、受契约门与 check/forget 门；orphan-newborn 路由 `q6` | `orphan-states`/`board-lint`/`entity-lint`/`node-lint`/`gap-lint`/`fold-lint`/`broadcast-*`/`activation-lint`/`orphan-newborn-island`/`orphan-forget`；`q6` 路由分区 | `runMemoryCheckTick (qct)`（`68573-68687`）、`"orphan-states"`（`68605`）、`"board-lint"`（`68615`）、`"orphan-newborn-island"`（`68641`）、`"orphan-forget"`（`68667`）、上限 `q6`）；`routeContractDecision (q6)`（`68506`）、`partition: q6(n)`（`68395`） | confirmed |
...
|---|---|---|---|
| **board-lint** | `gve(u, q6)`）→`yve`（`66768`）（逻辑 `ylt`（`66832`）） | `revise.v1` / `merge.v1`（判据见 B3） | REVISE→pattern-tracker，MERGE→intuition-weaver |
| **entity-lint** | `wve(u, q6)`）→`Sve`（`66932`） | `entity-converge.v1` | intuition-weaver。判据：entity 缺收敛三段 `vve = ["What it is now", "Relationship", "Open variables"]`（`66978`）；打分 `kb*1e3+dated`（`66948-66953`） |
| **node-lint** | `kve(u, q6)`）→`xve`（`67009`） | `node-converge.v1` | pattern-tracker（判据见 B3） |
| **gap-lint** | `Jve(e.eventsDir, u, t, s, …)`）→`Zve`（`67712`） | `scan-gap.v2` | gradient-distiller（见 B3） |
...
| **orphan-newborn-island** | `d(gwe(f, _));`）+`bwe(_we(f), t, …)`（`68644`） | `orphan-newborn.v1` / `orphan-islands.v1` | NEWBORN→`q6` 选择，ISLAND→intuition-weaver（见 C） |
...
**B4 · 每类每 tick 至多一条（`q6=1`）。** 常量 `q6`）取值 1，是每类 lint 的默认 limit：`yve`/`Sve`/`xve` 都 `slice(0,n)` 只取**最差 1 条**（help 的 `--limit=N default 1`）。即每 tick 每类最多投一个 worst-first 信号——这是"测量廉价"的核心节流。
...
**B6 · 前置短路：没有下游读者就不测量（v0.8.1 复核：候选分区集已扩大）。** `jve`(`67417`)在跑任何 lint 前遍历所有 `Object.values(Un)`（全部 lint kind）× `P6`，现为 `['pattern-tracker','gradient-distiller','intuition-weaver','memory-weaver']`——**已从 v0.7.1 时代的两分区扩到四分区**，与 §B7/`q6` 处"MERGE→intuition-weaver、SCAN_GAP→gradient-distiller"的路由改动一致），任一 kind 过闸即测量（`GP(i,r,e.flagFallback)===null`）；`qct`（`68594`）内 `if(!a && !r) return o`——"有没有订阅者"是是否测量的前置门。
...
**D1b · v0.7.1：partition inbox 有了显式的"快照对账"语义，防止信号被静默漏处理或重复处理。** 新增的 inbox-sync 子步骤（`Mve`（`67374-67415`），从 `runMemoryCheckTick (qct)` 内经 `V6("inbox-sync", …)`）调用）在每个 memory-check tick 结束时：对受追踪分区列表 `P6`，现为 `pattern-tracker`/`gradient-distiller`/`intuition-weaver`/`memory-weaver` 四个）中的每一个，重新跑一遍契约门 `iO` 算出"这一 tick 的 lint 产物里，哪些 pending 文件现在仍然可投递"，与"这一 tick 实际新写入的文件"取并集，跟持久化在 `<inboxDir>/.memory-signals.json` 里的**上一次快照**做差集比对——不在新集合里的旧快照项，其磁盘上的 `.pending` 文件被物理删除（视为"分区契约已不再消费它，不该继续占着 inbox"），然后把新集合写回快照文件。这正是 changelog "partition inboxes now have defined snapshot semantics" 的字面机制：inbox 不再是"只增不减、靠 agent 自己 ack 删除"的松散目录，而是每 tick 都对账一次的显式状态（confirmed，逐行核对）。
...
| `q6=1`：每类每 tick 至多一条 worst-first | `slice(0,n)`，help `--limit default 1` | `q6`）/ `--limit=N (default 1)`（`cli.pretty.js:66027`） | confirmed |
...
**§1 认知装配**：`Xv`=resolveMetaPromptText、`Ahe`=renderJobMissionBlock、`Nhe`=renderPromptLayers、`Jh`=buildSystemPromptForChannelConfig、`Uye`=extractSystemPromptAppend、`eke`=buildTransientUserBlocks、`WEe`=transcludeBroadcastBoard
**§2 Turn/Drain**：`Qd`=appendDrainRecord、`Cb`=summarizeDrainRecords、`Wh`=isAbortLikeError、`Ef`=createAgentSdkAdapter、`zye`=computeCodexTurnUsage、`EH`=batchDrainItems、`Xw`=handleDrainError
**§3 Session**：`dse`=rehydrateSessionState、`KSe`=drainSessionMailbox、`LS`=computeInstructionsFingerprint、`OA`=runInstructionsFingerprintGuard、`Agt`=createSessionManager、`Wgt`=createMetaSession、`Jgt`=sweepTombstonedSessionRecords（`spawnSessionActor`/`wakeSessionActor` 是 `Agt` 内的闭包，无独立导出名：分别为 `N`（`Agt`@`83931`） 与 `se`）
...
**§5 Gateway**：`Kle`=appendBeforeExecuteGateway、`cO`=DAEMON_TOKEN_ENV_KEY、`Myt`=isLoopbackBindHost、`jyt`=resolveRemoteListenerConfig
**§6 Cadence**：`mB`=PARTITION_CORE_TOOLS、`kwe`=resolveCadenceIntervalMs、`Zgt`=runCadenceTick、`yJ`=scanAndSpawnDueJobs、`Ygt`=createJobScheduler、`Qgt`=createOutboxDeliveryManager（旧的 cadence 队列文件读写函数已不存在——v0.8 起潜意识改经 `duoduo spine cat/show` 读 Spine 事件，全仓再无 cadence queue 字面量）
...
**§8 运行时抽象**：`Nf`=isCodexAvailable、`ag`=resolveCodexSandbox、`Sc`=checkCodexAvailability、`IV`=ensureAgentsMdSymlink、`qye`=buildBaseInstructions、`Bye`=buildDeveloperInstructions、`xV`=buildCodexTurnInput、`yw`=createCodexAppServerAdapter、`OV`=isGrokAvailable、`kc`=checkGrokAvailability、`vw`=createGrokAcpAdapter、`Yut`=resolveRuntimePaths、`Rdt`=initializeRuntime
...
| 广播板包装 khe/hrt/grt | 在 `Nhe` 内 `grt.test(d)`（`55106-55112`），常量定义 `khe`） |
| 广播板 transclusion | `82197-82203` (`WEe`=transcludeBroadcastBoard)，薄封装，递归体已拆到内部函数 `JEe`@`82212`，`fgt`(maxDepth=5) 定义 `fgt = 5`（`82342`） |
...
| lint 主循环 `qct` | `runMemoryCheckTick (qct)`（`68573-68687`），短路 `a = Fct(…); if (!a && !r)`） |
...
| cadence 间隔（运行时常量） | `T0e("ALADUO_CADENCE_INTERVAL_MS",222e4,1e3)`）；仅为 status 展示串的 `"2220000"`（`87619`） |
