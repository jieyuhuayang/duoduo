...
| 适配器选择 `$e(w)` | `w.runtime !== "claude"` → `w.adapter`（未建则抛 "refusing to fall through to Claude"）；`w.origin !== "channel" \|\| !s.createStreamingQuery` → 一次性 `s`；否则惰性长驻 `w.streamingAdapter`（经 `x(w, P)` 建） | `$e`（`83790`）；`createSessionManager (Tgt)`（`83790-83821`） | confirmed |
...
- **state.json 运行时相关字段**：`sdk_session_id`、`pending_fork_to`（drain 起点写入 `forkFrom`）、`model`、`model_runtime:"claude"|"codex"|"grok"|"pi"`、`pending_model_fork:boolean`（读取：`forkFrom: n?.pending_fork_to`（`72333`），在 `Rg`（`72326`）内；写入：`setSessionModel` 四个 runtime 分支 `createSessionManager (Tgt)`（`85289-85386`）；codex 的 `pending_model_fork` 由 `kft`（`72304`）在 drain 起点 resolve）。
...
> 无 container 相关控制面方法（v0.6.0 起 container 模式退役，`container` 在 daemon 仅剩 `e.runtime_mode !== "container"`（`31542`）一处向后兼容类型守卫）。`/effort` 等斜杠命令不是独立 RPC 方法，经 `channel.command` 入站处理（`$b`（`87166`）→ `appendBeforeExecuteGateway (Gle)`（`87174`）；`/effort` 识别 `if (e === "/effort") return "/effort"`（`87078`）、intent 归 `"config"`（`87130`）、网关命令执行器 `FXe`（`87465`）内分支 `t.name === "/effort"`（`87680`））；每会话推理力度落到 run-config 字段 `"effort" in t && t.effort && (r.effort = t.effort)`（`55219`）/ `reasoningEffort: F.reasoningEffort`（`63503`）。
>
