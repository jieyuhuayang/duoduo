...
// symbol: verifyClaudeCodeRuntimeAvailable  (minified: Ohe, daemon.pretty.js:55039)
...
function verifyClaudeCodeRuntimeAvailable() {
    ...
    let o = She(import.meta.url),
        s;
    try {
        let l = o.resolve("@anthropic-ai/claude-agent-sdk");
        s = She(l)
    } catch {
    ...
    try {
        if (!_rt(u).isFile()) throw new Error(`[agent-sdk] platform binary at ${u} exists but is not a regular file`)
    } catch (l) {
        throw l instanceof Error && l.message.startsWith("[agent-sdk]") ? l : new Error(`[agent-sdk] platform binary at ${u} is not accessible: ${l.message}`)
    }
}
