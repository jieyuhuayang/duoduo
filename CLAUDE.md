...
- `docs/AGENT_INTERNALS_ANALYSIS.md` — the primary artifact: 8 runtime subsystems reverse-engineered from the minified bundle (note: this doc's §1–§8 taxonomy and `first-party/`'s 12 `NN-` directories are two different groupings of the same symbols, numbered independently — neither is derived from the other), pyramid-structured, every mechanism claim names the code behind it (`真名 (短名)`, or a snippet bound to a real name) and carries a `confirmed` / `未证实推测` confidence tag; older passages still carry line numbers, which are being phased out. Cross-validated against the reconstructed source.
...
- **Beautify fidelity**: the shipped minified bundle and `*.pretty.js` have identical ASTs (`tools/ast_equiv.mjs` with no map). Without it every other proof starts from a file that does not ship.
...
- **Committed = generated**: the committed `recon/`, `maps/` and `first-party/` equal what the pipeline generates for the version they claim (`tools/promote.mjs`).
- **First-party tree honesty**: every `first-party/**/*.js` body is exactly its symbol's whole declaration in `recon/daemon.recon.js` (not merely a substring of it), its header agrees with the rename map and with `maps/inferred_daemon.json` on whether the name is inferred, and its cited line IS the symbol's declaration line (`tools/verify_first_party.mjs`). Nothing else checks this tree, and every way it can be wrong is silent.
- **Citation identity**: every doc citation of the form `真名 (短名)` names a symbol that still exists and still has that short name (`tools/verify_citations.mjs`), and every name-bound snippet `代码片段`（`真名`） lies inside that symbol (`tools/check_bare_anchors.mjs`). Legacy line numbers are counted against a ceiling that may only fall; a drifted legacy line on an F1 citation is regenerable with `--fix`.
...
#    fidelity (shipped minified ≡ pretty) → export blocks → module gate → rename →
#    syntax → AST-equivalence → symbol index → rename tables + first-party tree,
#    then compare everything with the committed copies, verify the committed
#    first-party tree, the citations and the legacy line anchors, and write
#    $OUT/pipeline_report.json (verdicts + toolchain versions included).
#    Bundles run concurrently; JOBS=1 forces sequential when bisecting a failure
#    (a failure fails the build in both modes). BEAUTIFIED=<dir> skips beautify.
#    The version is read from $PKG/../../package.json; a PKG_VERSION that
#    contradicts it is refused. Without PROMOTE the run never writes into the repo.
...
fails the build when the committed `pipeline_report.json` claims the same package version
(stale or hand-edited), and is reported as a pending retarget otherwise. `PROMOTE=1` writes
them in; there is no manual copy step any more. Run it after a bump once the inferred map
has been reviewed, then `verify_inferred.mjs record`.

**Doc retargeting has one correct order, and the line step is one-shot.** (Name citations need none of this — only `retarget_symbols.mjs` for short names. The steps below exist for the legacy line numbers that remain; every one converted to a name form is one less to move.)
...
- **Cite code by name, never add a line number.** Every mechanism claim names the code that implements it and carries a `confirmed` / `未证实推测` tag; never present an unverified inference as fact. A line number is not stable evidence: esbuild re-mangles every identifier and the formatter reflows on every release, so most lines move on each bump, and at v0.8.3 about two hundred of them could not be relocated at all and had to be dropped. A real name comes from esbuild's `__export` table and survives releases; the symbol index (`maps/symbols_*.json`) and the `first-party/` tree already say where it is. Two citation forms, both checked by the build (defined in `reconstruction/tools/anchor_forms.mjs`):
  - `` `真名 (短名)` `` — a symbol in `maps/symbols_*.json`, e.g. `atomicAppendEvent (on)`, **with no line**. `verify_citations.mjs` fails the build if the symbol is gone or the short name is wrong, wherever the pairing appears (prose, tables, diagrams). The short name is kept because it is what a reader greps for in the pretty bundle; `retarget_symbols.mjs` updates it on a bump.
  - `` `代码片段`（`真名`） `` — "this statement proves it", e.g. `` `stage: "runtime_mismatch"`（`drainSessionMailbox`） ``. `check_bare_anchors.mjs` requires a distinctive token of the snippet (a string literal, or a non-keyword identifier spelled as in the pretty bundle) and every short name the snippet calls to lie inside that symbol's current declaration. Quote string literals where you can: they survive releases, while a quoted mangled identifier changes on every bump and is then refuted until someone fixes it.
- **A function with no real name gets one before it is cited.** Derive a name, record it in `maps/inferred_daemon.json` (then `verify_inferred.mjs record`), and cite it in the first form. A bare short name (`` `jXe` ``) cannot be checked by anything and silently turns into a different function on the next release — v0.8.3 reused `AXe`, `UEe` and `WEe` for unrelated code. An unrecorded inferred name is just as bad: that is how `classifyModelContextRequirement` sat in the docs for versions without existing anywhere in the reconstruction.
- **Line numbers are legacy and may only disappear.** The older forms — `` `真名 (短名)`（`N`） `` (F1), `` `短名`（`N`） `` (F2), `` `代码片段`（`N`） `` (F3) — are still checked while they exist, and any other line number (plain-text `daemon:N`, several lines in one span, a number inside a ``` fence, …; `looseLineNumbers()` finds them) is unbound. `reconstruction/maps/bare_anchor_baseline.json` holds two per-doc ceilings, `lineNumbers` (every line number, whatever its shape) and `unbound` (held at 0), and `check_bare_anchors.mjs` fails the build when a doc exceeds either — so adding a line number, even a correct F1 one, fails. When you touch a paragraph that has line numbers, convert them to the two forms above, then lower the ceiling with `check_bare_anchors.mjs … --baseline maps/bare_anchor_baseline.json --write-baseline` in the same commit. `--write-baseline` refuses to raise a ceiling; `--allow-raise` exists for a genuinely new doc, not to make room. When a claim's evidence cannot be re-found, drop the citation and, if nothing else supports the claim, downgrade it to `未证实推测`. Diagrams in fences carry no citations; cite in the prose below them.
